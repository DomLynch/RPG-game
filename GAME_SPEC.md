...
Slice J — opponent psychology (owner's DNA table, For Honor "reads / psychology", 2026-09-16). The warden counts the player's habits from committed state edges only (`AiState.habits`: guard ticks, parry presses, rolls, cuts vs heavies, its own swings) and turns them into **reads** with evidence thresholds (`READ`): *parry-happy* (parry presses on ≥ 50 % of its swings after 3), *turtle* (guarding ≥ 45 % of the match after 3 s), *roller* (rolls on ≥ 40 % of its swings after 3), *spammer* (≥ 70 % cuts after 6 swings). Adaptations, all bounded: a spammer is parried at double the profile's chance (cap .85; a profile that never parries still never parries); a turtle is kicked more (chance .5 → .8, score 1.1 → 1.4) and charged through more (+.4); a roller sees heavies held to the charge even with no guard up (the roll's i-frames expire) and gets swung at in a roll's tail; a parry-happy player sees lights *held past the parry window* as baits (12 held ticks vs a 10-tick window) and more charged heavies. The first exchanges are always honest — no read fires without evidence — and the debug overlay prints the habits and active reads.

Slice H — hit impact (owner's DNA table, Mortal Kombat "hit impact", 2026-09-16). **Hit-stop**: the frame loop freezes the simulation on a contact tick while frames keep rendering — Blocked 30 ms · Hit 50 · Parried 70 · heavy-class Hit (heavy, charged, riposte-heavy, guard counter, critical) and GuardBroken 90 · PostureBroken 120 · Killed 220. Wall-clock only: ticks are never skipped or replayed (the accumulator is emptied at the contact tick so a dropped-frame phone shows the contact pose and resumes without a jump). **Camera kick**: a blow nudges the camera 2 cm (4.5 cm heavy-class) along the blow's heading and settles in ~0.15 s; off under `prefers-reduced-motion`. Both obey readable brutality — nothing covers a pose; the freeze makes the pose *more* legible. Placeholder for the visual lane's impact pass (sparks, decals, recoil are theirs).

Slice G — posture (owner's DNA table, Souls/Sekiro "poise", 2026-09-16). Every fighter has a posture bar (0–100, `RULES.posture`): a blocked hit puts the move's `posture` on the blocker (light 18 · thrust 14 · heavy 30 · riposte-family 20–30 · kick 24; a perfect block takes half), a clean hit puts it on the victim (×1.25 on a counter-hit), and being parried puts 35 on the attacker. It drains 12/s while the fighter is not staggered. Full = **posture break**: the bar resets, the victim staggers for 90 ticks and the other side gets a 90-tick **critical window** in which Heavy is `critical` (heavy-riposte path, 40 damage, unparryable, armoured, breaks guard) and Light is the ordinary riposte; starting any attack consumes the window. A guard break resets the victim's posture (that was the payoff). The warden gives ground while its bar is ≥ 70 % and finishes a broken player with the critical. HUD: a thin amber bar under each health bar, red at ≥ 70 %; hints warn at 70 % and call the critical. Measured AI-vs-AI (24 seeds): normal median 20.9 s / 7 hits (22.9 s before), 3 breaks in 24 duels; hard 18.9 s / 8 hits, 14 breaks, 4 kills by critical; easy 0 breaks — posture rewards sustained pressure without shortening duels below target. Weapon families (table item 6) are parked by the owner; the longsword loop comes first.

Slice C — thrust and the weapon-disc control trial (owner + external review, 2026-09-15). **Thrust** (`thrust`, T on a keyboard, ↑ on the disc): the Riposte clip's path at 16/5/21, damage 14, 25 stamina, a walking-pace lunge (stepIn 1) so it lands from 2.0 m where a cut needs 1.75 m and a heavy needs 32 ticks; fully blockable (chip 0, 25 stamina), parryable, a riposte inside the punish window; the spacing and counter-hit tool, never the guard opener. **Chamber**: every plain cut, thrust and heavy has a `chamber` tick (6 / 8 / 10) at which a *held* swing pauses for up to RULES.charge.max ticks — the load is the tell — and from which it can still be feinted; only a move that `charges` (the heavy) gains hyper-armour there and becomes the charged swing after RULES.charge.min held ticks. A chambered light is a bait, not a guard breaker: the warden blocks it and keeps a planned block. **Control trial** (menu → Controls, persisted per device): *buttons* (v0, today's Light/Heavy/Step/Guard/Kick); *disc · flick (v1)* — a stroke on the weapon disc is the strike (← → cuts, ↑ thrust, ↓ heavy; diagonals resolve to the nearest axis), never chambers; *disc · drag & release (v2)* — the stroke loads the swing at its chamber, release strikes, returning to the centre feints, a long hold releases itself before any charge; *disc · drag & release · hold to charge (v3)* — v2 plus the heavy's charge. *v4 invisible field* (the right half of the arena as the attack surface, a ring only under the thumb) was built, played by the owner and **dropped** the same day — it stays out of the cycle. *thumb cluster (v5)* — round buttons curved around the right thumb's rest, sized by frequency: Slash 66 (alternates sides itself), Stab 50 above, Heavy 50 right, Kick 40 inside, Step 56 and Guard 64 below; Stab and Heavy hold to load/charge like the keys. *segmented disc (v6)* — one 150 px disc in four sectors, tap toward one: ↑ stab · ← slash · → heavy · ↓ kick (so no Kick button), the thumb staying down loads or charges; Guard and Step beside it. Commitment in every grammar is *duration* (how long the thumb stays down), not stroke length: the engine has no horizontal heavy, so "long flick = committed slash" would need new moves. Guard and Step stay buttons in every scheme. **Decision 2026-09-16: the thumb cluster (v5) is Frankendom's mobile control.** Buttons are round and mostly see-through (≈32 % ink, thin bone-coloured ring, small-caps labels, pressed and disabled states), curved around the right thumb: Slash 66 · Stab 50 · Heavy 50 · Kick 40 · Step 56 · Guard 64. The flick disc (v1) stays as the one alternative in the menu; the square grid, drag/charge discs, invisible field and segmented disc are retired (git history keeps them; a stored retired scheme falls back to the cluster). Icons in place of labels are the visual lane's when the set is stable. The journal shows a per-scheme scorecard (fights, wins, rematches, average duel length, damage dealt/taken) — the owner's "would I duel again after ten fights" number. Direction chooses the attack; stroke length and time choose commitment. Diagonal cuts, an uppercut (needs a rig clip), hammer/spear/axe disc grammars and a contextual kick are later.
