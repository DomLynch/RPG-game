...
test('the control trial survives storage that is missing, corrupt or hostile, and round-trips through save', () => {
  ...
  assert.deepEqual(loadTrial(memory(JSON.stringify({ scheme: 'sectors', card: { flick: { fights: 'x' }, cluster: { fights: 1, wins: 1, rematches: 0, ticks: 900, dealt: 100, taken: 20 }, drag: { fights: 2, wins: 0, rematches: 0, ticks: 1, dealt: 1, taken: 1 } } }))), { scheme: 'cluster', card: { cluster: { fights: 1, wins: 1, rematches: 0, ticks: 900, dealt: 100, taken: 20 } } }, 'a retired scheme falls back to the cluster and its tally is dropped');
