...
test('the control trial survives storage that is missing, corrupt or hostile, and round-trips through save', () => {
  ...
  const storage = memory(), trial = loadTrial(storage); trial.scheme = 'ring8'; recordFight(trial, 'ring8', true, 1800, 100, 43); recordRematch(trial, 'ring8');
...
test('the scorecard reads as one line per played scheme with average duel length in seconds', () => {
  ...
  recordFight(trial, 'ring8', false, 1500, 60, 100); recordFight(trial, 'ring8', true, 2100, 100, 70); recordRematch(trial, 'ring8'); recordRematch(trial, 'cluster');
  assert.equal(formatCard(trial), 'thumb cluster — 0 fights · 0 won · 1 rematches · 0 s avg · dealt 0 / taken 0\nguard ring · v8 — 2 fights · 1 won · 1 rematches · 30 s avg · dealt 160 / taken 170', 'no wall-clock recorded: simulation seconds only');
