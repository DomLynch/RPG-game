...
test('the ladder is the encounter order minus the held recipes: the four creatures wait for after beta, the five men keep their order', () => {
  assert.deepEqual(LADDER.map(o => o.id), ['veteran', 'pitborn', 'goblin', 'nightborn', 'executioner', 'dwarf', 'witch']);
  ...
  assert.equal(nextAfter('dwarf')?.id, 'witch'); assert.equal(nextAfter('dwarf')?.name, 'the Witch');
  assert.equal(nextAfter('witch'), undefined, 'the last live rung offers no next opponent: the creatures are held');
