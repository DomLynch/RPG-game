...
test('Disarmed append preserves existing GLB bytes and clips, is reproducible, and refuses to erase later animations',async()=>{
 ...
 try {for(const rig of ['warrior','veteran']){
  ...
  const loaded=await new GLTFLoader().parseAsync(JSON.stringify(parsed),'');for(const name of ['Death_Disarmed','Fin_Disarmed']){const clip=loaded.animations.find(c=>c.name===name)!;assert.ok(clip.tracks.length>30 && Math.abs(clip.duration-3.2)<1e-6,'app loader reads complete paired clips');}
