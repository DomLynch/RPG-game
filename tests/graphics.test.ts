...
test('fight end: the rank line replaces the death-screen autopsy on a loss, the autopsy lines go under the opponent\'s journal row, and a rematch clears the rank line', () => {
  ...
  assert.equal(app.element('fight-rank-label').textContent, 'Recruit I · ○ ○ ○', 'the account panel\'s label, no save text');
  assert.equal(app.element('fight-rank-next').textContent, 'Legionary', 'the next class sits at the end of the pip row');
