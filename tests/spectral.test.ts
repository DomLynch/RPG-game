...
test('enlarged Wraith strikes stay at human torso height and reset without scale or arm drift', async () => {
  ...
  const actor = buildWarriors(asset, asset, ['estoc', 'estoc']).opponent;
  ...
  for (const attack of ['light', 'return', 'heavy', 'thrust', 'riposte'] as const) {
    const spec = attackSpecs('estoc')[attack], contact = spec.contact / spec.recovery;
  ...
  const fresh = buildWarriors(asset, asset, ['estoc', 'estoc']).opponent;
