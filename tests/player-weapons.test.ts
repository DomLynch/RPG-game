...
const KNOWN_UNFAIR = [
  // After the warden reach fix (combat/warden-reach, 2026-09-21): 11 rows → 7. Trident and warhammer came clean everywhere and are offered.
  // What is left, by cause: cleaver/executioner — a pre-existing cut-tempo row (Weapons: CLEAVER light 22/8/26 vs the sword's 20/8/22);
  // knife/veteran and scythe/veteran — a poker parked just inside the Veteran's own reach at range; knife/goblin hard — three fights out of
  // 24 still run out the clock against the guardless Goblin; estoc/goblin ×2 and estoc/dwarf hard — the estoc's move table sits .3–.4 m short
  // of its blade bake (tests/weapons.test.ts "real reach"), so every warden misjudges its point until Weapons corrects ESTOC_MOVES.
