...
test('weapon flip: the record carries the weapon; an older record version is refused; an unknown weapon is refused; the replay verifies on that weapon [slow]', async () => {
  ...
  // A version-1 stream (magic, version 1, build, opponent, profile, seed, 0 ticks, outcome, no weapon field) and version-2/3 streams
  // (the same with the weapon) are all refused: the rules moved under them (#371, #366, brief 8), so decoding one would replay a
  // different fight.
