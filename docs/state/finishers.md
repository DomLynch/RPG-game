...
## Now (Finishers & Gore lane, 2026-09-25 night; Lead orders the queue)

**Now — Knight dent B soften (live defect, Strategy ruled (a))**: branch `finishers/knight-dent-b` @ 7372faa1 (pushed, no PR yet; trunk c1bda34d merged in). `src/signature-knight.ts` darkDent() = one soft radial falloff, core alpha .8 (stops .8/.7/.3/0), scale .25 via a per-look `size` (A keeps .42), no strokes: the bright top arc and the hard wall were the "spinner" under the per-hit tilt. Test `tests/signature-knight.test.ts` (+1: size .25, max stop ≤ .8, edge 0, zero strokes; B back at .42 → red). quality:stop was green at .6 (632/0/2); **re-run tsc + npm test on 7372faa1**. Owed to Lead: ONE full 375×812 frame of the struck Knight, before (trunk) vs after. PASS = Strategy sees a dark dent on the chest without hunting and it is not a disc; if it fails, that is failure two and the concept moves to Rivet A, no more tuning rounds. Then open the PR (tsc + npm test, no SIM_FILES), queued after #756.
How to shoot it: apply `lane-notes/harness-knight-dent-signature-turned.patch` (untracked, adds `--signature B` and `dent-*-left/right.png` stills orbited ±0.6 rad, because at the straight play camera the hero's head and hammer hide the chest) to finisher-preview in both trees, set the wounds viewport to 375×812 locally, then run `node scripts/finisher-preview.mjs --only plainDeath --wounds --signature B --opponent knight --no-video --label knight-dent` in a trunk worktree and on the branch. The 0.3 s `-left` still is the frame (before shows Dom's black disc + white arc). Harness patch = local only, never committed.
