...
**NEXT, in order (Lead's plan, 19:00):** wait for Lead's FREE after deploy 36d4aecc. Then (1) ONE hand-driven Dwarf plain kill on live at 375: driver `dwarf-drive.mjs` on branch `evidence/kill-cam-sweep` @ 30c7bad8. Copy it to a scratch dir, symlink node_modules, fix its harness-clock import path, then `node drive.mjs &` and append commands to `./cmd` (`hold KeyW 200`, `press KeyQ`, `hold KeyQ 48`, `press KeyT`, `run 16`, `beat` after the kill, `quit`); read `./out`. Controls, from quiet-one-browser-check: W advance, Q guard (tap ~3 ticks before contact = parry), T thrust/riposte, F strike, S step back. Record the panel/buttons px² series and whether head/wound are hidden; if hidden, report to Lead (Strategy decides). (2) rows 16/21/28 (`quiet-one-browser-check --opponent goblin`, `--finisher opened --opponent goblin --blood-check`, `--finisher decapitation --opponent goblin --blood-check`) on the branch head after `npm run build`: the new assertion runs inside them. (3) `node scripts/quality-stop-targeted.mjs` alone (last pass 705/0 was before the assertion commit). (4) Open the sweep PR (body: assertion, plain-death hold, Dwarf note, Quiet-One regex gap, stills link) and send the head to Lead. Load < 30 before every run; check the lock AND Lead's slot.

**Now — kill-camera sweep** `finishers/kill-camera-sweep` @ 40102ac5 (pushed; trunk merged; assertion 8d24528b; not yet a PR). quiet-one-browser-check samples the fallen body's box against the loot panel and its Take/Leave buttons every 0.1 s over 0–1 s, then every 0.5 s to 10 s, and asserts: never over the buttons; never over the panel, except a plain death's first 3.5 s. Goblin at 375x812: splitCrown and opened 0 px² throughout. runThrough/decapitation 0 in the 09-25 sweep.
