...
**NEXT:** rows 16/21/28 on the branch head, then `node scripts/quality-stop-targeted.mjs` alone, then the sweep PR, READY <PR#> <sha> to Lead. The kill-cam sweep is CLOSED for beta (Strategy 2026-09-26) unless a new opponent's still shows a head or wound covered.
