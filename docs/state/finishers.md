...
## Now (Finishers & Gore lane, 2026-09-23 morning)

**Next — START HERE: blood Option 3, Dom's pick 2026-09-23** ("start with recommendation option 3"; Lead will brief on top — fold that in when it arrives). Dom, on a live phone screenshot of the hero's back: blood "still looks like paint-ball graffiti stickers… less uniform, differentiated each blood spot… more dripping style not a star looking shape… should move downwards or drop from body onto the floor, like droplets… not a river, rather slowly". Ownership settled: Dom asked whether this is World's; Strategy confirmed it is this lane's (the body wounds are ours — see the entries below).

Diagnosis (grounded, 2026-09-23): (1) `impactTexture(true)` in `src/scene.ts` (~l.190) is ONE procedural star — radius `33 + sin(7a)*6 + cos(11a)*4` plus 17 satellite drops evenly on a ring — shared by the floor splat pool, the wound decals and every body mark until the photo loads; only `rotation.z` varies, which cannot change a radially symmetric shape. (2) The photo that replaces it on the body, `src/assets/blood/blood-splat.png` (256² RGBA, properly alpha-cut, ships in dist), is a top-down POOL on a floor with radial spray — right for the ground, wrong for a vertical torso — and it is one image for all 10 marks. (3) It reads near-black: `FRESH #7a2a2c` is multiplied over an already dark-red photo (`src/gore.ts` ~l.201).

Build order (Dom approved the order): **step 0** measure whether runs are clamped to ~0 on the PLAYER (side 0) — the screenshot shows no runs at all, while the Veteran measures 0.06 → 0.12 → 0.13 m (`finisher-preview.mjs --only plainDeath --wounds`, re-run 2026-09-23, exit 0). Suspect `surfaceReach()` (#455) on the hero tunic; the harness only drives hits on the warden (PASSIVE profile), so add a hero-hit window (warden on `OPPONENTS[id].profiles.normal`, player idle after the draw) and read `view.probe().bodyWounds[0]` at +0.3/1.5/3 s. If clamped, that is a bug under the art and is fixed first. **Step 1 (Option 2)**: 4–6 wound splats authored for a vertical surface (soak-in patch, irregular non-radial edge, wet core, feathered, NO satellite ring) + 3 drip variants via the FLUX gradio_client route; per-hit pick from the existing `woundSeed`/`lcg` so replays match; mark top edge = the cut, taller than wide; lift the tint off black. Kill the 17-drop ring. Dom judges on his phone before step 2. **Step 2 (droplets)**: a run that hits its reach cap sheds a drop every ~1.5–3 s (seeded), falls under gravity, lands as a SMALL floor mark — the existing `createSplatPool.splash` is 22×13 cm, far too big, needs a small size class; hard cap in flight; phone budget check (the pool is 12 slots).

**Lead's brief (2026-09-23, matches the above; full text in `docs/state/lead.md` on PR #517 — beta item 5, "blood follows bodies")**: (a) = step 0, (b) = step 1 as ONE PR, (c) droplets = a SEPARATE later PR carrying a **CPU ×4 throttled frame-budget receipt**. Reporting line: this lane reports to **Lead** now (Strategy rules, Lead dispatches) — each report is PR number, head sha, gate numbers, screenshots. If Dom's word and Lead's differ, Dom's wins; tell Lead what differs.

**In flight, not mine to merge**: PR #506 `finishers/finisher-complete` @ d122825 on trunk fe0d8e0 — loot panel waits for the kill to finish playing. All gates green (receipts on the PR); Strategy put it in Deploy's code batch ahead of #505 (both touch `src/main.ts`).

**Parked**: kill-camera framing on `finishers/kill-camera-hold` (this branch), measurement-only (`quiet-one-browser-check.mjs` records the fallen rect + open panel box at the latch, asserts nothing). Measure on top of #475 (it pins the loot card to `min(calc(100vw - 32px), 360px)`), not trunk. Separate PR after #506 (Lead's sequencing; Strategy agreed).

**Open, not closed**: Nightborn head-slot mark −2.7 cm at 0.3 s and MISS at 1.5/3 s — cause unknown, bind-pose bounding sphere disproved.

**Gotchas**: `vite preview` serves the last `npm run build` — after a deploy, `dist/` is trunk's, and a browser row reports the new feature as missing (`complete=undefined`, panel open at the kill). Build first. The one-deployer guard also refuses single-file `node --test` while a deploy is in flight, despite its message.
...
## Finisher complete: the loot panel waits for the kill to finish playing (Lead brief 2026-09-22; Dom on the phone, live 607126a: "I have never seen the decapitation land")

`src/scene.ts`, `src/main.ts`, `src/finishers.ts`, branch `finishers/finisher-complete` on 2eacfe8, PR #506. `main.ts` fired `offerLoot()` on the `Killed` event, so the panel opened over the ceremony.

**The event.** `view.finishPhase()` gains `complete` and `completeAt` beside `settled`/`touring`/`age`. The latch is composed of what the scene is actually doing, never a delay: the victim's clip has run out (`victimProgress` — the 0.75× presentation clock for a posed finisher, the plain fall's own progress for a plain death, which is why the plain death is shortest), the camera has settled (`camera.ts` SETTLE), and Decapitation's severed head has come to rest. Opened needs no term of its own: its reach freezes at clip end by construction. It latches once and holds until the finish clears, so a late camera nudge cannot un-complete a ceremony the player has watched end.

**Measured durations** (seconds from the Killed event, Veteran, seeds 731–748, by `node scripts/finisher-preview.mjs --durations`, which plays each captured kill window frame by frame on the production path and records the frame the latch fires on): runThrough 4.12, splitCrown 4.07, quietOne 3.77, decapitation 3.20, opened 3.20, plainDeath 2.40. Published as `FINISHER_SECONDS`; **the runtime does not read it** — the game keys on the event. `hamstrung`/`execution` have no clip, so `finisherSeconds()` returns the plain death's figure flagged `measured: false`. Receipt `artifacts/character/finisher-durations/finisher-durations.json`.

**The hush moved too, and this is the part worth keeping.** While a loot offer is pending, `endgame-fade` holds to `complete` rather than `settled`. The faded Rematch row is inert, the fade used to lift at ~1.9–2.0 s, and the ceremony runs to 2.40–4.12 s — in that gap the row is live, the ceremony is still playing, the panel has not opened, and every reset path clears the pending offer. A player pressing Rematch there silently loses the loot that win earned, with nothing having told them an offer was coming. Same class as the action row in the thumb zone (#427). Do not tighten it back without closing the same hole.

**Decisions taken by others, recorded so they are not relitigated.** Strategy initially briefed a ~1.5 s hold after the event and a plain kill firing at settle; asked for the measurement behind the 1.5 s, they withdrew it in their own words ("my number, not a measurement") and accepted the clip end for the plain kill, one rule for every outcome. If Dom wants a beat, Web adds it in the panel PR as his judged value, named as such. Lead ruled the camera framing a separate PR: its receipt is a phone screenshot stuck behind the same blocked window, and stacking a blocked receipt on an unblocked PR blocks both while Web waits on the event. Strategy did not override that.

**Two corrections issued to Web**, both acted on premises that were false: the branch *was* on origin as #506 (they were holding their push to avoid publishing my commits), and a browser gate *does* see this timing on a real win — `quiet-one-browser-check.mjs` is a win path ("win by normal controls", asserts `#target-health` 0), not a death path, so the debug-only win seam they asked Strategy for is not needed. `endgame-hud-check.mjs` is the death path they were reading.

**Evidence.** `tests/finishers.test.ts` 7/7 (per finisher not one constant; every rotation outcome has one; the plain death is shortest; each past the settle floor; the two unshipped labelled derived). `test:all` 507 / 505 pass / 0 fail / 2 skipped; `tsc --noEmit` clean. `--durations` folded into the existing blood-gate release row, which already runs all six outcomes — no extra CI time; it asserts the latch fires, sits past the settle floor, and agrees with the scene's own finish age. `quiet-one-browser-check.mjs` asserts, on a real UI win with the finisher selected, that the panel is closed at the kill and open at the latch — release rows 15, 20, 25 cover quietOne, opened, decapitation.

**Remaining validation, stated not hidden**: `quality:stop`, the blood-gate row and the three quiet-one rows have not run on this head — deploy fe0d8e0 in flight since 19:14Z. They run on the next FREE window, and the framing measurements above come out of the same run.
