...
**Morning PR after Rivet A — black floor oval (Lead, 09-26 00:25)**: Web's #772 still (`/tmp/web-772-swap-witch-kill-375.png`, Witch kill at 375, ≈(240,1150) of 750×1624) shows a flat PURE BLACK oval beside the body, no rim; World ruled out its scatter (#780). First read (still only, no browser run): a hard ~14-sided polygon, which matches the drop spot's `CircleGeometry(0.5, 14)` (gore.ts:345), whose sand-multiply map is set only when the floor maps load (gore.ts:367). Suspect: a spot drawn without its map. **Downgraded 07:25 (09-26)**: measured, the oval is 113×47 px at (185–297, 1131–1177), pure black (2,1,1), ~30 cm across. Drop spots are 4–8 cm and draw dark red #681a19 before their maps, and the splats are square planes shaped by texture. The faceted rim points at a real disc mesh (arena.ts has 12/14-sided cylinders). Next: raycast through that pixel in the same kill frame and print the mesh name and material, also on live; no speculative fix; hand over to World if it's theirs. To do: a same-frame A/B (marks on vs off) to confirm, then fix it so it reads as blood on every arena, using the Rivet A / #752 rules. Browser runs only after 02:00 with load < 30. Lead's accepted fix shape: never draw a drop spot before its map is ready, either by hiding it until the floor maps resolve or by giving it a non-black blood fallback. Plus a test that a spot created before the maps load is either invisible or carries the fallback. Web's still came from a localhost preview, where map timing can differ, so run the A/B on live frankendom.com as well.

**Queued — release row 32 (wounds gate) flake (Lead, 09-26 06:57)**: failed exit 1 at load 182.5 (06:17, run fffe8cf, 108 s) and load 308.9 (06:49, run 4c1d6af1, 114 s); passed both times rerun alone (52 s at load 83). The failing assertion is LOST: the solo retry overwrites `artifacts/release-checks/32-*.log`, and the runner prints a tail only on final failure. Ruled out by reading: every wounds assertion runs on the fixed frame clock; the CPU ×4 budget is only logged; the 120 s waitForFunction can't be it. Suspects: Playwright's 30 s defaults on `page.goto` and `page.screenshot` (never raised). Proposed to Lead: (1) move the budget behind `--budget`, off the release row, and give goto and screenshot 120 s timeouts; (2) Deploy's call: keep the first attempt's log on a retry. Lead APPROVED (1), SKIPPED the ×6 repro (box time goes to Combat's RV14 battery before 16:00) and is asking Deploy for (2). (1) is DRAFT **PR #792** `finishers/row32-timeouts` @ 36a44ba8 on trunk 0325a0b7: --budget opt-in, context.setDefaultTimeout(120000) on both contexts; finishers.test + release-checks.test 16/16. Owed before READY: row 32 run locally on the head (exit 0) and one --budget run, in a FREE window with load < 30 after RV14. The PR must not claim to fix the flake.
