...
**In flight, gates owed**: PR #506 `finishers/finisher-complete` (head 8d106e6 on 2eacfe8) — the loot panel waits for the kill to finish playing. MERGEABLE; `base` SUCCESS on the runner, `quality` and `release-checks` queued (`release-checks`'s first attempt was cancelled before a single step ran — `cancel-in-progress` on the concurrency group in `.github/workflows/release-checks.yml`, not a failure; re-run). **`quality:stop` and every local browser row are UNRUN on this head**: deploy fe0d8e0 has held them under the one-deployer rule since 19:14Z, nine refusals. `test:all` 507 tests / 505 pass / 0 fail / 2 skipped and `tsc --noEmit` clean both passed before the deploy started, and are the only local evidence that exists. Not mine to merge.

**Also in flight**: PR #455 `finishers/blood-conform` (head 42496eb on trunk c43c677). Handed to Lead, MERGEABLE.
...
**Next**: the kill-camera framing, started on `finishers/kill-camera-hold` (6f352bc, off #506's head) and deliberately measurement-only so far — `quiet-one-browser-check.mjs` records the fallen body's screen rect and the open panel's box at the moment the panel opens, per finisher, and asserts nothing. The only figure available about today's framing is `endgame-hud-check.mjs`'s own comment (feet at y ≈ 865 on an 844-tall frame, i.e. the body runs off the bottom) against a brief wanting it above y ≈ 487; that gap is too large to design across without measuring, and a gate written first would encode today's framing as correct. The check runs at 390×844, not the brief's 375×812, so the line is 60 % of the live `innerHeight`, with the viewport recorded beside it.
