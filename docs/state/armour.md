...
**Dom's acceptance test (via Lead, 16:1x):** "It should be visibly cooler per level, to keep players interested. No one wants to collect trophy pieces that look like shit." Per slot, rung N+1 must read as MORE desirable than rung N at the fight camera AND on the Profile; mark each adjacent pair pass/fail on the ten-rung sheet's index. Readability bar (Strategy, provisional): changed share ≥ 15 % AND (|value delta| ≥ .08 OR silhouette ≥ 3 %); calibrate on the Doctor's hat (reads) vs the Goblin's body (does not); the harness computes the three numbers (`--rungs`, readability block).

**Where things stand (17:2x).** #831 d6734541 and #836 bc4a81a7 are READY with Deploy in the run #834 → #837 → #838 → #831 → #836; do not push to those heads. **Crest slot** is built on branch `armour/crest-slot` (stacked on #836, draft PR): key + migration + unit test (pass and deliberate fail in the PR body) + worn-loot-check row + layer render done; STILL TO DO at FREE after that run: `npm run build` (done once, stale-safe to redo), 375 + 1280 Profile stills with helmet AND crest worn (crest card visible without scrolling at 375, nothing pushed off), browser rows profile-figure-check + loot-smoke-check + worn-loot-check, gate, then un-draft and READY to Lead. Web restyles the card (session was unreachable; details in the PR body). Then, in Lead's order:
1. (done) loot-layers reproducibility → #836.
2. (in flight) Crest gets its OWN paperdoll slot (Lead ruling a): `crest` key in `src/loot.ts` PAPERDOLL, Crest out of `head`; a cleanLoot migration moving `equipped.head = '<x>.Crest'` to `equipped.crest` (never lost; owned/pack untouched) with a unit test and a release row; Web adds the Profile card (coordinate the slot id); Backend confirmed the server needs nothing (loot_claims regex accepts `<opponent>.Crest`, no key names checked). One older build open elsewhere drops the key until it reloads. Stills 375 + 1280 of the Profile with helmet AND crest worn.
...
6b. split-loot carrier pruning (Lead 16:2x): every carriers-*.glb carries every loot image; prune each to the images its own draws reference, check-budget before/after in the PR.
...
Rank-tint retune (after #835, now on trunk): the Knight's arm + shoulder plates stay silver on every rung (find why: material id or slot key), the Veteran's near shin never changes (body mesh: leave; greave: grade), greaves/body read faintly at the fight camera (push value contrast per rung), reshoot the Shieldmaiden in Arena 1; handoff note docs/proposals/rank-tint-handoff.md. Ten-rung standing acceptance sheet (Dom's order): `node scripts/armour-contact-sheet.mjs --rungs --label <trunk sha>` after `npm run build`, ≈1.5 h render, parts ≤ 2,800 px + index.md with the pair marks; first use: confirm/deny the three defects above; sheets → Lead → Strategy → Dom.
