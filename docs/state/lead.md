...
## Lead — 2026-09-26 14:36 +04: Sparring republish 93b1a768 in flight (row 28/38); next run 822+817+705+docs READY; rank-tint sheet 17:00 (read this first)
**Dom 13:5x: Lead owns every lane, gate, READY and CI question; Strategy gets milestones only** (one msg per live sha, one per slip with cause + new time within 5 min, one at 16:30 delivered-vs-pending in Dom's words). Lanes messaging Strategy → "through Lead". Strategy clears after the Sparring sha line; wake it by messaging the Strategy lane.
**Now.** LIVE still **acdbe355** (my curl 14:36). Deploy running **93b1a768** = 0adc7214 (#815 Sparring, #814 Audio drop-H, #813, #812) + cc587ea2 (#818 witch test) + #821 (harness); log ~/Developer/deploy-93b1a768.log at row 28/38 14:36. Target live **14:50** (slipped twice: 14:05 → 14:30 = #815 moved #finisher-select to Options, quiet-one rows 16/21/28 failed → Lead's one-line #821; 14:30 → 14:50 = Lead ordered Combat's Jab-reach sweep without "after FREE", load 79). On live: curl release.json + bundle, send Strategy the sha line.
**NEXT RUN (all heads final, Lead-gated, READY with Deploy; launch after 93b1a768 is live and every head fully green):** #822 Sparring dummy @ **b758ebdc** (Web; Combat's e7d97ac0 folded in, scoped digest e1ac7cc5; 4 files, none in SIM_FILES; quality SUCCESS 14:36, 7 jobs running) → #817 Audio guard_break @ **b7795bc8** (all green, verified) → #705 tier dressing @ **f85246b6** (World; ruling-C PASS by CM; + Doctor hat fix NOT_WORN plaguedoctor Helmet — hat was sheen on flat crown, not asset/runtime; 11/11 glbs = cdf94a36, verified; quality SUCCESS, 3 running) → docs #819 (Multi Chars) + #820 @ ea61ad81 (Combat). merge-tree clean between all. Targets: dummy ~15:15, #705 + #817 + hat 15:30.
**Owed today (Dom's outcomes):** (5) Export clip B live 16:30 — Web fresh session, head by 15:45. (6) Finishers #823 DRAFT @ 953607cc (Goblin lock camera = #752 redo, 1 s smoothstep fade on finish start; needs row 25 Goblin+Dwarf + rows 16/21/28 + 375 A/B stills) → run after #705; then kill-cam sweep (splitCrown/opened/plainDeath × Goblin+Dwarf). **Rank finishes (Strategy GO):** Character Main on char/rank-tint off #705 57203aa8 — map-preserving tint (metal/trim/leather, never cloth), graded by Provenance.tier; opponents by own rung; 375 ten-rank Centurion sheet **by 17:00**, NO PR until Dom judges; 2 retune rounds budgeted. CM state at 14:4x (CM cleared): char/rank-tint @ e6a23dd4 on #705 f85246b6, LOCAL unpushed in CM app worktree; unit 3/3, typecheck clean; draws/programs Master = Origin = 101/55 (Recruit 99/54, no crest); sheet NOT shot yet, waits for FREE (front camera ?look=foe); CM lane memory char_lane_handoff_2026-09-26-afternoon; 17:00 at risk if the box stays busy. Strategy cleared 14:4x: handoff atop docs/state/strategy.md on strategy/state-0926; first msg to fresh Strategy = Sparring live sha + bundle.
**Closed today:** Cleave lever (no change); Reaping v Shieldmaiden 84% closes with it; Jab best-window (no hole; weak rows Goblin normal 46, Wraith normal 31); Jab reach (NO change: 1.2/1.4 fail fairness estoc v Goblin +18/19, Centurion neutral 76→48); Reaping "blade in body" (camera occlusion, Executioner's tick-stamped side-on frames); Doctor metallicFactor PR (dropped, wrong diagnosis); creature-check stale stamps (PARKED post-beta, no hand re-stamp; Doctor re-fit on warrior.glb 59c8b27e→48efd44a parked); #816 closed (superseded by #822); witch.glb CI red (#818: V8 Math.pow 1-ulp Node 22 vs 25; JSON to 12 sig digits).
**Open:** Jab live zero at 11:32 walk-in run — undiagnosed, needs Web's press-gap tick log (after clip B), no speculative change. Gear table for Dom: pieces covered Recruit→Origin after #705 (Dwarf no greaves/boots, Veteran no shield, no crest at Recruit); finishes at ZERO ranks (never called) → the rank-tint sheet.
**Rules added today:** any PR that moves/hides/renames a UI element greps scripts/ for its selectors and runs the browser release rows before READY (Strategy standing rule). Every battery/sweep order says "after Deploy posts FREE, max 4 shards".
**Gotchas.** Lead session sits in app worktree funny-rhodes-f0eaa1 (Dom's 13:3x OK covers edits/pushes from app worktrees); Dom still to reopen Lead on ~/Developer/frankendom-lead-catalogue (uncommitted docs/state/lead-catalogue.md there, −558/+45, not Lead's — ask before committing). zsh: quote `"$REF:path"` (unquoted `$T:s…` is a history modifier).

## Lead — 2026-09-26 13:30 +04: all ten skills live (RV15), Knight iron + Witch staff B live; Sparring + Export clip next (read this first)
**Now.** LIVE **acdbe355** (12:43; my curl: witch-DmHwiSq9.glb). Box FREE, nothing READY. Next, both Web: **Sparring mode** (Dom 13:1x: Options-menu entry, picker opponent/difficulty/weapon/special, fight-only overrides, zero writes, admin-only behind SPARRING_FOR_ALL, ?daily=1 pattern, no sim change; Finisher moves Settings → Options; plan accepted, head +75 min from start), then **Export clip B** (SHARE opens LINK/CLIP; last 12 s of the record, 720x1280, game audio, no HUD; WebM only / fixed 12 s / share sheet only; new MediaRecorder wiring, reuse main.ts REPLAY_TAIL; +55 min). **Audio**: drop hit take H (overdriven "fizz") from hit_flesh + hit_heavy, test pin 6 → 5. Dom 13:3x (via Strategy): lanes may edit and push from their app worktrees.
**Done today (verified by my curl).** a0c71273 10:46 = RV15: all 10 opponent skills + Pommel; Lunge d11 / Iron Rush d10, stagger 0, stamDmg 0; Jab re-offered (first run 52411fea failed row 36: loot-smoke tapped a fixed pixel the new Jab tile covered → #807 taps above #loot-panel, elementFromPoint guard). d74f1632 12:12 = #795 strict-replay refusal (x64 trig drift: --strict is Mac-only in practice). 76f70ae2 12:25 = #809 Knight iron (was a black mirror: baked loot maps had metallicFactor 1; Knight .4, Dwarf .35) + #808 + #810. acdbe355 12:43 = #811 Witch staff B (Dom's pick, saturated green stone reads at 375 READY; node-children swap, 125/126 glb objects identical).
**Rulings.** Battery PASS = 480 seeds, bar Pommel+40, cleared by ≥1 sd (48 seeds cannot tell an inert skill from Pommel). The .5/.35 win cap is NOT a bar (live Pommel breaks it v Executioner: post-beta look). Skill-hole rule: a skill with no usable timing v an opponent is a hole (best-window counter >80%); 50–80% Strategy decides. Item 4: each newest opponent's special IS its attack style for beta. Item 7 kick: no change (player kick spam wins 0/480 except Goblin 7%).
**Open.** Combat OFFLINE since ~12:03 (cleared at 411k; needs Dom's relaunch): Cleave lever (recovery counter v Goblin 89% easy = hole; target <50% normal / <70% easy v Goblin, Shieldmaiden, Nightborn, Doctor; poise ticks, then shorter tell, never damage) → Jab best-window table → Sparring "dummy" AI profile. World: #705 re-cut of carriers-knight.glb from the 76f70ae2 loot.glb, then CM's ruling-C PASS/FAIL, then #728 Executioner. Backend: #621 / #778 / #751 / #749. Web clips 5/9 on evidence/move-clips @ 3c28fdf1; Cleave clip needs Web's tick log (sim shows 0 trades) once Combat is back. Executioner: Reaping Blow 375 pose/weapon check.
**Gotchas.** A merged PR cannot take a fix commit: new PR. A new loot offer can move the panel under a pixel tap. Live-harness "trades" the sim cannot produce are harness until tick-stamped. Never add a pass bar Strategy has not ruled. change_directory is refused for app-worktree sessions. Verify every live claim with your own curl.

## Lead — 2026-09-26 06:30 +04: five releases overnight, NINE MOVES tonight as one batch, NO launch stops (read this first)
**Now.** LIVE **fffe8cf9** (06:25; my curl: index-CeHSS9g7.js, v:13, loot-BoDKvj5O.glb 200). Deploy is gating **RUN 2** = #781 9ebdf995 (Shieldmaiden tunic back) + #782 6083caf9 (Pitborn sash, pitborn.glb only) + #680 8ba1ed37 (player bot) + #779 b2e40da3 (Witch-fire scorch), tree 225c736f. Then **RUN 3** = #783 0f1e605d (player trident draw, 5 files, no SIM_FILES) → #784 c09424ba (scythe carry, stacked; retarget to trunk after #783). All READY, CI green, verified by me. After each: curl + bundle grep → sha line to Strategy.
**NINE MOVES (SCOPE 8), Dom re-ruling: ONE batch, ONE RV bump 13→14, READY 16:00, live tonight.** Rows = Combat's scope8-rows.md (in Combat's scratchpad; caps: dmg floor 18, Pommel-only follow-up, block-punishable ≥9) with 4 ruled flags: Cleave no breaksGuard + staminaDamage 60 + chip .4; Jab chained-light 16/8/18; Iron Rush poise 24 from tick 8; Miasma one-tick cone staminaDamage 50. SkillIds (move = skill_<id>): lunge (Nightborn), reaping (Executioner), shove (Centurion), jab (Goblin), cleave (Pitborn), stomp (Dwarf), miasma (Plague Doctor), ironrush (Knight), hewer (Shieldmaiden). Owners: **Pitborn** (fresh session; handover pitborn/state-0924-night 94d4d774) all rows + SKILLS + tests + skill-battery.mjs `--skill` (identical to pommel-battery on pommel), rows ~11:00; **Combat** reviews + ONE battery (~15 min, 3 shards) + ceilings + re-pin, Deploy holds the box for it; **World** skill-impact kit (world/skill-impact-kit, src/skill-impact.ts, ships on Pommel too; YES 16:00) — then #705 (ruling-C fix local ff2989d0); **Web** #785 thumbs (8 PASS, Reaping → scythe redraw). Failing row: fix numbers + re-run that shard; drop only if unfixable by 20:00. Character Main phone clip per move when live. Bespoke VFX (Miasma, Stomp, Reaping) later, not gating.
**Owed to Strategy/Dom:** capacity answers (Pitborn/World/Web YES; Combat battery 15 min) → tell Strategy by 08:30 that all owners make 16:00, risk = the box during runs 2–3. Character Main: Witch-fire clean body-hit clip + take-panel swap clip by 09:00. Wake table (Dom is up; can be a short status). Weapons: Witch mage staff A/B stills (visual trident variant, NO spike: a Witch kill plays no finisher), after warhammer/maul carries.
**Done since 09-25 22:53 (all verified by my curl).** e2a52a48 Pommel Strike RV13 + #765 + #774; bc12a665 audit #768/#771/#767/#770 (Auditer drove A+B live); 5d95a691 #769 Witch-fire v5 + #706 shield + #772 swap panel; fffe8cf9 #777 + #734 + #780 + #759. Accepted: #778 (client claims, blocker fixed via my_standing pending — publish with #621/#751/0001 after playtest; skill-take null-piece WIN MUST COUNT). Loot.glb chain: #716 (Multi Chars, after #734 live) → Pitborn sash PR 2 (loot.glb Gambeson splice) → Goblin #776 → #728 (behind #705). #783 trimmed to player trident (opponent carry parked on weapons/pole-draw-creatures e3cd3b27; opponents stay ready — Strategy).
**Open.** Hooks lane: the Stop quality gate orphaned runs at its 420 s deadline + all lanes' gates piling up drove load 200+ at 00:05 (evidence sent; Strategy relays). Drift audit (Auditer, #773 entry): parts-stage drift from faa89c46 (hero knuckles) moves the Pitborn kilt up to 37.6 mm — Character Main post-playtest; no parts re-runs for hero/veteran/executioner/pitborn/nightborn until settled; Nightborn greave f5118b69 rebuild post-playtest. Finishers: Rivet A, then the black drop-spot oval (gore.ts:345 CircleGeometry drawn before its map). Weapons: warhammer/maul carries, staff, pommel hilt-bash clip.
**Gotchas.** DOM RULE: NO LAUNCH STOPS — the queue runs 24/7, Lead self-wakes hourly at night, only Dom names a stop (the 02:00 stop idled 8 green PRs 4 h). Stamp times from `date`, never estimate. A peer's CI-state list is not a PASS; only an explicit ruling is. A PR's CI that runs "no release rows for this diff" (e.g. #734/#781 glb changes) is first tested by Deploy's 36-row run — say so in the READY. Lanes' batteries/test suites under the deploy lock slow the release (pids 71504, 72062 this morning) — tell the owner, don't kill their work. The deploy guard hook blocks Bash commands whose text looks heavy; write files via Write + cp.
...
## Lead — 2026-09-23 17:5x: CLOCKS MOVED TO TONIGHT (Dom: "it's 5pm we have 6 hours"). SUPERSEDES the tomorrow clocks below; the assignments stand.
**PHASE R, LIVE 21:00:** branch **`phase-r`** (Lead-pushed = trunk 7b277fd, the #566/#567/#571 batch merged). PRs go INTO phase-r; Lead reviews → READY → **Deploy merges into phase-r** (the one-merger rule holds) → Deploy publishes phase-r at 20:30. Clocks: Auditer's LOOT number 18:30 (**DONE: #585, LOOT 2 → 3.5 MB**; +36 pieces ≈ 3.0–3.1 MB at ~49 KB/piece dist; loot is not in the 12 MB per-fight; Lead READY; Deploy merges after the Auditer's receipt) | Nightborn's weld + textures 19:00 | Web's ONE re-render 19:00 | Weapons' Gladius + Longsword 19:30 | every lane's opponents 20:15 (one that misses rides Phase L's run). Rules sent to all: base phase-r, merge before push, never force, loot.glb is GENERATED (`WARRIOR_LOOT=1 node scripts/build-warrior.mjs` from src/assets/source/loot/loot.json + source parts), so REBUILD it after merging and never hand-merge the binary. Pitborn has started prep. **PHASE L, LIVE 23:00:** Stats owns the code (PR against trunk by 22:15; publish 22:30): characters.ts gradeMaterial(material, tier) CLONES (wear() uses the rig's own shared material) + applies grades.ts gradeFor; the player's worn pieces are graded by tierAt(marksOf(profile)); no sim files, no Provenance change; WORN_FROM stays empty (awards only). **SCOPE FINDING → RULING ASKED:** opponents NEVER wear loot.glb in a fight (they fight in their own body GLB, scene.ts:158), and the creature-pipeline bodies are one baked *Surface material, so they can't be regraded. (A) recommended: grade the PLAYER's worn pieces + the palette-material opponent draws; (B) opponents in rank material = wear their loot kit over stripped bodies, not tonight. **Reports to Strategy:** 19:00, 20:15, 21:00 (sha), 22:15, 23:00 (sha).
**17:4x UPDATES:** **LIVE `7b277fd`** (the batch #566/#567/#571; deploy.sh EXIT 0 17:47; rows 0 failed; release.json verified by Lead; sha line sent). Box FREE until 20:30. **Strategy RULED (A)** → forwarded to Stats (player's worn pieces + palette-material opponent draws in create(opponent); creature *Surface bodies skipped). **(B) = tomorrow's PHASE M** = Veteran's runtime equip loader: first use is the Centurion gladius+scutum re-land, second is every hero-rig opponent wearing its six in its rung's tier material. Scope it with Veteran in the morning; the clocks come from that scoping. #585 has its receipt (4/4 + tsc); Deploy merges it into phase-r. **Lanes told: don't wait for the weld PR; cut now with 817828e's scripts/character/loot_dwarf.py.** Veteran: dwarf.glb already holds Helmet/Body/Arms cuts, and loot.json took only Greaves; SLOT_OF sent foot/ball to Greaves (so no Boots was possible), which Veteran is fixing. Multi Chars: the Witch's robe/cloak are leg-skinned and land in Greaves; the fix is to partition by distance from the leg axis, and the FALLBACK (accepted) is 4 slots tonight + Greaves/Boots in the Phase L run. **GOTCHA:** #575 was merged (728b8c7). State commits now go on `lead/state-0923-1815`.
**18:0x:** phase-r tip 9047842 = #585 (LOOT 3.5 MB) + #586 (Weapons: shieldmaiden.Gladius + plaguedoctor.Longsword as EQUIP files, not loot.glb draws; 'Longsword' in WEAPON_SLOTS; NO_LOOT_YET removed) + #579 (Audio: six flesh-hit sounds on rotation, retargeted into phase-r). **KNIGHT ARMS DEFECT, LIVE since 322bb1d:** his creatures.py arm-edge line put the arm cut-off outside his hanging arms, so the arms carry zero arm weight (Executioner's histogram: 8,482 verts on spine_02, ~220 on the arms). **Lead REPRODUCED it in a live fight on 7b277fd** (?opponent=knight, HUD hidden): the gauntlets stay at his hips through guard, heavy and kill. Strategy ruled (a): Executioner's `hold: true` PR into phase-r (re-pin WITHOUT a bump under #439 + a record-replay-check --strict receipt) ships at 20:30, so 9 rungs tonight. The depth-aware arm re-weight targets the 23:00 run (ceiling tomorrow morning); the un-hold, new knight.glb and his six ship together, with a mid-swing still. Character Main: the Goblin is a parts.py body (no helm/greave/boot geometry to cut), so he gets 3 authored parts.py kit pieces and his FIGHT GLB changes (per-fight budget + clip rows + a mid-fight still asked). **Repro tip:** `frankendom.com/?opponent=<id>` boots that fight; hold keys with dispatched KeyboardEvent keydown/keyup (single key presses don't move); hide the touch overlay with a `body *:not(canvas){opacity:0}` style to see the fighters.

## Lead — 2026-09-23 17:4x: PRIORITY 1 = SIX PIECES ON EVERY OPPONENT (Dom via Strategy; Lead runs it). READ THIS FIRST.
**GOAL:** all ten ladder opponents WEAR and OFFER six takeable armour pieces (head/chest/arms/hands/legs/feet, SCOPE.md) + their weapon; no "none" on the ladder table. **PHASE R (Recruit rag & scrap), LIVE target 2026-09-24 14:00:** (1) PREREQUISITE: Nightborn's seam weld (char/plague-doctor-loot @ 817828e) + textures as its OWN PR by 09:00, Lead reviews, own publish; no quick-cut carriers. (2) One PR per opponent on the welded pipeline; its body carries the six listed, tri counts, loot.glb size, a same-frame phone still WEARING them (Dom judges), and loot-layers green. (3) The paperdoll re-render/frame widen is done ONCE (Web) before the first Phase R merge. (4) Auditer raises the LOOT cap with a number if needed. **START (LOOT @ 5c0a32c):** Centurion 6 (+Crest, Shield) | Executioner 6 (+Crest) | Nightborn 5 (no Greaves) | Goblin 3 | Pitborn 2 | Dwarf 2 | Plague Doctor, Knight, Witch, Shieldmaiden 0. **ASSIGNED (all told 17:4x):** Nightborn = the weld by 09:00, then Nightborn + Plague Doctor | Pitborn = Pitborn + Shieldmaiden | Executioner = Knight | Multi Chars = Witch (acked, #577 ace6eca) | Veteran = Dwarf (+ the equip loader/Centurion re-land order by 09:30) | Character Main = Goblin | Weapons (fresh session, handoff #581) = shieldmaiden.Gladius + plaguedoctor.Longsword (+ 'Longsword' in WEAPON_SLOTS) | Web = the ONE re-render, ETA by 09:00 (+ opponent cards) | Auditer = the LOOT cap estimate at six-piece sets by 10:00. **PHASE L (Legionary leather), target 2026-09-24 20:00:** the six repainted leather = tier material at RUNTIME (the #514 kit resolver wired into the game + WORN_FROM data; grades.ts/gear-stats.ts have NO runtime importer today). A mechanism + one material pass. **Lead scopes it tomorrow morning** (the Stats/Web split). **REPORTING:** a table to Strategy every 3 h from 09:00 (opponent | pieces 0–6 | material | PR | still y/n | blocker) + sha lines. **RULED (a) by Strategy 17:5x:** Phase R = six MESHES per opponent (the Centurion's and Executioner's Steel sixes count as done). Phase L = the material mechanism applied at Recruit (rag & scrap) AND Legionary (leather) in the SAME PR, so Dom sees rags on rung 1 and leather on rung 2 at 20:00. Phase R stills show base material, and each PR body says "material at Phase L". **Relay this to the lanes at 09:00** (they stopped before the ruling). Web flag: the "316→352" frame figure is not on trunk (it came peer-reported from the Executioner's knight.Helmet attempt), so Web measures the real frame constant + the widest new piece itself. **ACKS (state docs with Phase R as Now):** Multi Chars #577, Web #562 (8326dcf), Auditer #568 (ce70c4a), Character Main #582 (5886b72), Veteran #583, Weapons #581, Executioner (Knight, branch char/executioner-state). Pitborn: see below. **LOOT SIZE (resolved):** check-budget measures DIST (scripts/check-budget.mjs `dist = argv[2] || 'dist'`, Node gzipSync default level) after scripts/optimize-glb.mjs, not src. The Auditer measured dist loot at 1,327,597 of 2,000,000 on 15d4e55 (src 5,066,536 raw → dist 2,480,544 raw). Lead's 2,180,657 was src at gzip -9, which the gate never sees. About 670 KB of headroom for about 40 pieces, so the 10:00 estimate is taken on the weld PR's own build. Nightborn plans one 512 colour+ORM atlas per opponent and flags early if ten × six won't fit. Nightborn ACKED (#576 9c08dce; restarting fresh; test piece = a knight.Helmet re-cut; first bake when Deploy posts FREE). Pitborn ACKED (#584; Pitborn + Shieldmaiden after the weld). The jaw band is still open (WIP on LOCAL pitborn/shieldmaiden-body @ 74de033, not pushed; the instrumented build runs when the box frees, before the loot work). All nine lanes acked. **TONIGHT:** only the #566/#567/#571 batch (7b277fd in flight; send its sha line), Weapons' two weapons, and Nightborn's weld prep; everything else writes state docs and stops.

**HANDOFF 17:3x — NOW:** (1) the batch #566 (be2e8e1) + #567 (15d4e55) + #571 (6efc7d3) is running on Deploy; when it's LIVE, send its sha line to Strategy. (2) **STRATEGY RULED 17:3x:** Weapons cuts shieldmaiden.Gladius AND plaguedoctor.Longsword into loot.glb as ONE non-sim PR after the batch (Weapons told; it needs 'Longsword' in WEAPON_SLOTS). After the batch nothing new starts tonight and lanes write state docs; Strategy's gear/weapons-per-rank brief comes tomorrow morning. (3) The SCOPE.md as-shipped fix = **#580** (docs only; Deploy told, along with #575/#576/#577). (4) Web's opponent cards + the "Five opponents" copy (public/game/index.html) come as a PR on trunk; Pitborn's jaw fix is a trunk PR after an instrumented build (visual glb only). Both ride the next non-sim batch. **DONE:** **LIVE `5c0a32c`** (Deploy 17:32, EXIT 0, rows 0 failed; release.json verified by Lead 17:33) = roster-v0 via #578 (a merge of c2a5c73, SCOPE.md from trunk's #573 line; src/tests/supabase byte-identical to c2a5c73, Lead-verified). The migration 202609230002 was applied BY NAME (Deploy's receipt: 14 ids; 0001 absent). The sha line + the 10-rung ladder table + the gear gap went to Strategy. Ladder = roster.ts order minus held: Centurion, Pitborn, Goblin, Nightborn, Executioner, Dwarf, Plague Doctor, Knight, Witch, Shieldmaiden. Takeable: Knight Maul and Witch Trident only among the four (Plague Doctor's longsword has no loot slot; there's no shieldmaiden.Gladius). KNOWN_UNFAIR = []. grades.ts/gear-stats.ts/awards.ts have NO runtime importer. **OPEN:** migration 202609230001 + the verify-loot unit (HELD); #559 + the Centurion re-land on Veteran's equip loader (tomorrow); knight vs warhammer normal 12/24 (on the cap, a watch row). **GOTCHA:** trunk's merge of roster-v0 conflicts only in docs (Deploy resolves it with trunk's line); check `git diff <reviewed> <merge> -- src tests supabase` is empty before deploy.sh.
**ROSTER-V0 READY @ `c2a5c73` (Lead review):** = origin/roster-v0 = origin/combat/bump8-roster; ancestors 80e991a, 0c5aa89, 9801276, 59d2436 verified; 80e991a..c2a5c73 = 7 files (moves: plagueDoctor normal lapse .2, OFFERED + cleaver + maul, #550's executioner normal + `anticipate`; ai.ts the `profile.anticipate ?? READ.anticipate` hook; record v8, READABLE [8]; guard SIM_DIGEST 6fb55940…; fixtures; tests). Lead ran on detached c2a5c73: 6 targeted test files 40/40, tsc 0. Combat + Multi Chars report quality:stop 525/0/2. Migration 202609230002 lists 14 ids (unchanged since 0f508f9). **Deploy told:** wait for Backend "OK 202609230002 @ c2a5c73" (asked), apply BY NAME (never db push; 0001 stays HELD), merge + deploy.sh as its own run, STOP if roster-v0 moved. Pitborn (jaw fix) and Web (opponent cards + "Five opponents" copy, NOT on c2a5c73) -> PRs on trunk after roster-v0, riding the next non-sim batch.
**COMBAT BATTERY (Combat-reported, local merge 64a62b1 = combat/bump8-roster + roster-v0 80e991a):** 160 rows; #550 clears maul vs executioner 18->7 and cleaver 18->8; the two plaguedoctor "untouched 3/24" rows fixed by ONE knob (plagueDoctor normal lapse .3->.2, worst row 6/24); KNOWN_UNFAIR EMPTY; watch: knight vs warhammer normal 12/24 (on the cap); opponents.test 20/20. **LEAD RULED (A): the MAUL IS OFFERED** (added to PLAYER_WEAPONS_OFFERED; the test's fair-means-offered rule, no exception): it was held back only for the executioner row, it matches the weapon-only takeable ruling (knight.Maul), and player/maul.glb + the hero blade table came with #572. Combat now does v8 + the re-pin + replay refs, pushes to roster-v0, and sends the head.
**ALL FOUR BODIES IN (Lead-verified `origin/roster-v0` @ `80e991a`):** shieldmaiden=gladius, knight=maul, plaguedoctor=longsword, witch=trident; all four .glb present. Multi Chars' 3-way silhouette PASSED (IoU W–S .578, W–V .672, S–V .620; option (b) holds). **Dom 16:5x via Strategy: Lead runs the roster-v0 publish END TO END; the clocks are CEILINGS, not targets.** Combat was told GO NOW: merge roster-v0 into `combat/bump8-roster`, run the battery, retune the four placeholder ARCHETYPES, handle the over-cap rows vs plaguedoctor (trident 3/24 charged-heavy row, scythe row: tune or sign "bump 9"), clear maul/cleaver vs executioner, the six identity pins pass, then v8 + the SIM_DIGEST re-pin LAST, and send Lead the head. Nightborn, Multi Chars and Weapons were told to report to Lead only. **NEXT:** Lead reviews the re-pin (ancestors 80e991a + 0c5aa89, v8, the digest, no signed row left vs the knife) -> Backend re-OKs 202609230002 at the final sha -> Deploy READY (apply 202609230002 BY NAME, never `db push`, with or before the client) -> the sha line to Strategy -> the freeze ends -> one batch #566 + #567 + #571 -> its sha line.
**18:3x STATUS + A DECISION DUE 19:30:** on roster-v0 (a71e208+): Plague Doctor (longsword), Knight (warhammer; switches to maul ONLY if his drawn mesh is rebaked to the maul, with a render), Weapons' MAUL merged (#572, not offered; its maul-vs-executioner row shares the cleaver cause and Combat clears both at the re-pin); Witch built (38/38 clips), pushing. **DIST BUDGET:** check-budget TOTAL = 31.45 of 32.00 MB gzip with 3 bodies, so the Shieldmaiden (~6 MB) WILL FAIL. Per-fight is fine (worst 9.16/12). Asked Strategy: **(A) raise TOTAL to 40 MB (per-fight 12 unchanged), owner Auditer, a one-line PR into roster-v0** vs (B) drop the held creatures. **CARRIERS PULLED (Lead 18:5x, Strategy told):** no Recruit-2 Helmet/Body pieces ship tonight for any of the four. Untextured-Steel .12 cuts on TRELLIS read badly (UV-seam splits tear), and knight.Helmet turned loot-layers.test RED (the fix re-renders all 31 layers and widens the frame 316->352). Executioner pulls knight.Helmet/Body (KEEPS knight.Maul); each ships its weapon piece only. The post-beta basis is Nightborn's weld on char/plague-doctor-loot @ 817828e. Knight = MAUL, drawn = sim verified (78657a9). Plague Doctor in characters.test FIGHTERS (f076d19). **Guard rule (Lead):** until Combat's 21:15 re-pin, record-version-guard is the ONE expected red on roster-v0; there's NO interim bump (combat/bump8-roster already has v8), and any other red is real. **LOOT cap 2 MB also ruled + landed (636ce4d)**; carriers go on shared untextured Steel at .12, optional tonight. On roster-v0: Plague Doctor, Knight (warhammer), Witch (3707dee); the Shieldmaiden is pending. **RULED (A) by Strategy 18:3x: TOTAL 40,000,000 gzip, per-fight 12 MB unchanged; Auditer commits into roster-v0 now.** (B) = a post-beta Auditer ticket.
**16:2x:** the PLAGUE DOCTOR's real body is on roster-v0 @ cf95e1e (Nightborn; id `plaguedoctor`: ids must match ^[a-z]{1,32}$; hero rig + longsword; last rung; the versus still is made). Migration **202609230002** (the fighter_profiles encounter allowlist) — Backend OK on conditions: the final file lists all 14 ids spelled as the roster.ts keys (Nightborn widens it after the last row lands, then Backend re-OKs at the final sha); **Deploy applies it BY NAME, never `db push`** (202609230001 awards is HELD and would apply early); apply with or before the client. Web adds opponent cards (dwarf + the 4) + "Five opponents" copy on roster-v0 by 20:45. #571 floor blood READY (gates green) for the post-roster batch. The expected red record-version-guard on roster-v0 waits for Combat's single 21:15 re-pin.
**16:04 LIVE `59d2436` = lighting option C** (sha line sent to Strategy; Dom checks on his phone). The box is FREE until roster-v0 at 21:20.
