...
create function public.fight_records_recent(uid uuid) returns integer language sql stable security definer set search_path = public as $$
  select count(*)::integer from public.fight_records where user_id = uid and created_at > now() - interval '1 hour'
$$;
revoke all on function public.fight_records_recent(uuid) from public;
grant execute on function public.fight_records_recent(uuid) to authenticated;
...
create policy "a fighter stores his own, thirty an hour" on public.fight_records for insert to authenticated
  with check (auth.uid() = user_id and public.fight_records_recent(auth.uid()) < 30);
