...
const guard = () => ({ ...idle(), guard: true }), passive = { ...PROFILES.easy, aggression: 0, parry: 0, dodge: 0, pressure: 0 }, light = { ...PROFILES.normal, pressure: 1 };   // pressure 1: the warden opens with a light cut
function ready(gap = 1.2, enemyHealth) {   // both fighters armed and facing, the hero \`gap\` metres from the target (tests/combat.test.ts)
  const p = initialPractice(731, OPPONENTS.veteran), f = p.duel.fighters;
  return project({ ...p.duel, fighters: [{ ...f[0], phase: 'ready', body: { x: 0, z: TARGET.z + gap, heading: Math.PI, distance: 0 } }, enemyHealth ? { ...f[1], health: enemyHealth } : f[1]] }, p.ai);
}
// Each moment: the practice states up to and past the contact tick; \`at\` is the index of the contact tick.
const MOMENTS = {
  block() { let s = ready(); const list = [s]; for (let i = 0; i < 900 && !s.events.some(e => e.type === 'Blocked'); i++) { s = stepPractice(s, guard()); list.push(s); } return tail(list, s, guard()); },
  lightBlock() { let s = ready(); const list = [s]; for (let i = 0; i < 900 && !s.events.some(e => e.type === 'Blocked'); i++) { s = stepPractice(s, guard(), light); list.push(s); } return tail(list, s, guard(), light); },
  parry() {   // look ahead to the contact tick under a held guard, then press the parry 5 ticks before it (the window is RULES.parry ticks)
    ...
    let look = s, n = 0; while (n < 120 && !look.events.some(e => e.type === 'Blocked' || e.type === 'Hit')) { look = stepPractice(look, guard()); n++; }
    for (let i = 0; i < n - 5; i++) { s = stepPractice(s, idle()); list.push(s); }
    s = stepPractice(s, act('parry', { guard: true })); list.push(s);
    for (let i = 0; i < 30 && !s.events.some(e => e.type === 'Parried'); i++) { s = stepPractice(s, guard()); list.push(s); }
    return tail(list, s, guard());
  },
...
await server.listen();
const url = `${server.resolvedUrls.local[0]}impact-preview.html`;
...
try {
  ...
  for (const [name, at] of [['block', [0, 2, 5, 9]], ['lightBlock', [0, 2, 5, 9]], ['parry', [0, 2, 5, 9]], ['heavy', [0, 2, 5, 9]], ['kill', [0, 1, 3, 8]]]) {
  ...
  for (const name of ['block', 'lightBlock', 'parry', 'heavy', 'kill']) console.log(`  ${name}: peak camera shift ${stats[name].peakShiftPx} px${previous?.[name] ? ` (was ${previous[name].peakShiftPx})` : ''}; trace ${stats[name].cameraShiftPx.join(' ')}; brightness ${stats[name].brightness.map(([f, b]) => `+${f}:${b}`).join(' ')}`);
