...
// so the rules must reproduce the reference fights in tests/fixtures/fight-records.json bit for bit: same final tick, same
// outcome, same Killed tick, same fighters (digest). Pure simulation, no browser, well under a second.
//
// Passes when every reference replays identically, OR when decodeRecord refuses the record's version cleanly (the format moved
// on: regenerate). Fails loudly on a silent mismatch — a rules change that alters a recorded fight breaks every shared link, so
// it must be a conscious decision: `node scripts/record-replay-check.mjs --write` regenerates the references and the PR that
// carries the new fixture is the receipt.
...
for (const { name, encoded, expect } of records) {
  ...
  const drift = Object.keys(expect).filter(k => got[k] !== expect[k]);
