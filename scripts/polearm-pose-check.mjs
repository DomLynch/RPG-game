...
const dir = process.env.POLEARM_POSE_DIR || 'artifacts/weapons/polearm-poses';
...
try {
  for (const [id, prefix, cut] of [['veteran', 'Trident', 'Sweep'], ['executioner', 'Scythe', 'Reap']]) {
    ...
    const poses = [['Idle', 0], ['Walk', .4], ['Guard', .9], ['High', .26], [cut, .34], ['High', .48], ['Thrust', .34], ['High', .72]].map(([clip, t]) => [`${prefix}_${clip}`, t]);
    const captures = [];
    for (const [view, azimuth] of [['front', 0], ['side', 270], ['rear', 180]]) {
      const data = await page.evaluate(([p, a]) => window.__preview.clipSheet(p, a, true), [poses, azimuth * Math.PI / 180]);
