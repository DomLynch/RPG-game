...
const selected=process.argv[2],families=selected?[selected]:['minotaur','wraith','werewolf','skeleton','dwarf'];
if(families.some(f=>!['minotaur','wraith','werewolf','skeleton','dwarf'].includes(f)))throw new Error('Choose minotaur, wraith, werewolf or skeleton');
...
for(const family of families){
 ...
 if(family==='dwarf'){const r=spawnSync('node',['scripts/build-warrior.mjs'],{stdio:'inherit',env:{...process.env,WARRIOR_FIGHTER:'dwarf',WARRIOR_WEAPON:'trident',WARRIOR_PARTS_VARIANT:'veteran',WARRIOR_OUT:'src/assets/source/creatures/dwarf-donor.glb'}});if(r.error)throw r.error;if(r.status!==0)throw new Error(`donor build failed (${r.status})`);}
