...
const VIEWS = {
  ...
  'brazier': { size: [900, 720], ratio: 1, place() { camera.fov = 51; camera.position.set(4.6, 4.6, 4.6); camera.lookAt(8.66, 3.75, 8.66); } },  // temp close-up of the π/4 brazier (flame vs pot)
};
...
try {
  ...
  if (args.includes('--moodboard')) {
    ...
    for (const view of ['brazier']) await save(`${view}.png`, await page.evaluate(v => __preview.capture(v), view));
