...
// none → legs=dwarf.Greaves → + chest=goblin.Body. Stills of the figure are kept as the receipt a person looks at. (A pixel diff was
...
const STATES = [['none', {}], ['over', { legs: 'dwarf.Greaves' }], ['over+replace', { legs: 'dwarf.Greaves', chest: 'goblin.Body' }]];
...
try {
  ...
  for (const [name, equipped] of STATES) {
    ...
    await page.waitForFunction((want) => JSON.parse(document.querySelector('#debug').dataset.worn || '[]').length >= want, want, { timeout: 30000 }).catch(() => {});
    await page.waitForTimeout(300);
    const draws = await page.evaluate(() => JSON.parse(document.querySelector('#debug').dataset.worn || '[]'));
    await page.screenshot({ path: `${dir}/${name}.png` }); await page.screenshot({ path: `${dir}/${name}-figure.png`, clip: FIGURE });
    receipt.states[name] = { equipped: kept, draws, lootFetched: lootFetches.slice(fetchesBefore) };
  }
  ...
  assert.ok(receipt.states['over+replace'].draws.every((d) => !d.endsWith('(hidden)')), 'no worn copy is detached or invisible');
