...
try {
 ...
 await page.getByRole('button',{name:'Menu and field journal'}).tap();
 // Controls cycles cluster → flick disc → guard ring (v7) → guard ring (v8); the stroke above ran on the disc. Walk back to the cluster and record the labels seen,
 // checking each ring's layout on the way: no two visible controls overlap, except Slash inside the Guard ring, which is both rings' design.
 const layoutClean=async(scheme)=>{for(const size of [{width:393,height:852},{width:844,height:390}]){await page.setViewportSize(size);const s=await snapshot();assert.equal(s.scale,1);assert.equal(s.overflow,false,`${scheme} overflows at ${size.width}`);const boxes=await page.locator('.actions button:visible').evaluateAll(nodes=>nodes.map(n=>({...n.getBoundingClientRect().toJSON(),id:n.id})));for(const box of boxes)assert.ok(box.width>=44&&box.height>=44,`${scheme}: ${box.id} is ${Math.round(box.width)}×${Math.round(box.height)} px, under the 44 px touch minimum`);for(let i=0;i<boxes.length;i++)for(let j=i+1;j<boxes.length;j++){const a=boxes[i],b=boxes[j];const nested=(scheme==='ring'||scheme==='ring8')&&[a.id,b.id].sort().join()==='attack-button,guard-button';assert.ok(nested||a.right<=b.left||b.right<=a.left||a.bottom<=b.top||b.bottom<=a.top,`overlapping controls (${scheme}): ${a.id} × ${b.id}`);}}await page.setViewportSize({width:393,height:852});};
 receipt.controls=[];for(let i=0;i<5;i++){const label=await page.locator('#controls-mode').textContent();receipt.controls.push(label);if(label==='Controls: thumb cluster')break;await page.locator('#controls-mode').tap();if(label==='Controls: weapon disc · flick'){await page.getByRole('button',{name:'Close journal'}).tap();await layoutClean('ring');await page.getByRole('button',{name:'Menu and field journal'}).tap();}if(label==='Controls: guard ring · v7'){await page.getByRole('button',{name:'Close journal'}).tap();await layoutClean('ring8');await page.getByRole('button',{name:'Menu and field journal'}).tap();}}
 assert.deepEqual(receipt.controls,['Controls: weapon disc · flick','Controls: guard ring · v7','Controls: guard ring · v8','Controls: thumb cluster']);
