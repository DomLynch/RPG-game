...
try {
  ...
  const sample = await page.evaluate(() => {
    ...
    return { fallen, topBand: pick(['combat-status', 'autopsy', 'loot-panel']), cluster: pick(['reset-button', 'share-button', 'loot-take', 'loot-decline']), actions: box(document.getElementById('actions')), resetOpacity: getComputedStyle(document.getElementById('reset-button')).opacity };
  });
