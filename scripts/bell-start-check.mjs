...
try {
 ...
 receipt.results=await page.evaluate(async(bellSeconds)=>{
  ...
  for(const scenario of ['returning','pauseBefore','muteBefore','mute','pause','late','death','rematch','mutedDraw','opponent']) {
   ...
   results[scenario]={bells,before:rms(0,2.4),attack:rms(2.51,2.65),afterCancel:rms(3.4,3.8),rematchWaiting:rms(5.2,7.4),tail:rms(7.5+bellSeconds+.1,8.5+bellSeconds),peak:20*Math.log10(Math.max(...data.subarray(2.5*48000,3.5*48000).map(Math.abs)))};
  }
