...
// day's seed, and the replay ends where the record says. Rows that fail stay grey (verified = false, never deleted) and are reported
// in the receipt, so a rules change or a forged record shows up on the next run.
//
...
// `node scripts/verify-daily.mjs --dry` replays without writing.
...
// One sweep over `db` ({ pending, seed, verify } — see restAdapter / psqlAdapter).
export async function verifyPending(db, { dry = false } = {}) {
  ...
  for (const row of rows) {
    ...
    if (reason) { receipt.refused.push({ day: row.day, user_id: row.user_id, reason }); continue; }
...
export function restAdapter(api, fetchFn = globalThis.fetch) {
  ...
  return {
    pending: limit => json(`${base}/rest/v1/daily_results?verified=eq.false&order=created_at.asc&limit=${limit}&select=day,user_id,opponent,weapon,outcome,ticks,record`, { headers }),
    ...
    verify: async (day, userId) => {
      const res = await fetchFn(`${base}/rest/v1/daily_results?day=eq.${day}&user_id=eq.${userId}`, { method: 'PATCH', headers: { ...headers, Prefer: 'return=minimal' }, body: JSON.stringify({ verified: true }) });
...
export function psqlAdapter(databaseUrl, run = spawnSync) {
  ...
  return {
    pending: async limit => JSON.parse(sql(`select coalesce(json_agg(r), '[]') from (select day::text, user_id::text, opponent, weapon, outcome, ticks, record from public.daily_results where not verified order by created_at asc limit ${Number(limit)}) r`)),
    seed: async day => { const out = sql(`select seed from public.daily_fight(date '${check(day, DAY, 'day')}')`); return out === '' ? null : Number(out); },
    verify: async (day, userId) => { sql(`update public.daily_results set verified = true where day = date '${check(day, DAY, 'day')}' and user_id = '${check(userId, UUID, 'user id')}'`); },
  };
}
...
if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  ...
  verifyPending(db, { dry: process.argv.includes('--dry') })
