...
export type ArenaFrame = { match: number; ended: boolean; tick: number; opponent?: OpponentId };
const URLS = {
...
export function createArenaAudio(context: BaseAudioContext, destination: AudioNode, now: () => number) {
  ...
  let buffer: AudioBuffer | null = null, match: number | undefined, random = seeded(1), opening = 0;
  ...
  function play(name: ArenaCue, gain: number, delay = 0) {
    if (!buffer || voices.size >= 6) return 0;
    ...
    source.buffer = buffer; source.playbackRate.value = rate; source.connect(envelope); envelope.connect(destination);
  ...
  return {
    ...
    update(events: CombatEvent[], frame: ArenaFrame) {
      ...
      if (match !== frame.match) {
        stop(); match = frame.match; random = seeded(match ^ 0x6172656e); opening = time; bellPlayed = false;
