...
import { selectFinisher, FINISHER_POSE, DISARMED_BEATS, type FinisherId } from '../finishers.ts';
...
export function cuesFor(events: CombatEvent[], presentation?: DeathPresentation): Cue[] {
  ...
  for (const e of events) {
    if (e.type === 'Hit' && finisher !== 'disarmed') impacts.push(e.move === 'kick' ? cue('hit_kick', .95, .2) : e.charged || HEAVY.has(e.move ?? '') ? cue('hit_heavy', 1, .3) : cue('hit_flesh', 1, .3));
      ...
      if(finisher==='disarmed') {
        const arm=DISARMED_BEATS.arm*DISARMED_BEATS.duration, neck=DISARMED_BEATS.neck*DISARMED_BEATS.duration+DISARMED_BEATS.hold;
        impacts.push(cue('whoosh_heavy',.18,.18,arm-.16),cue(gore?'flesh_cut':'hit_heavy',.65,.12,arm),cue('death_voice',.4,.1,arm+.04),
          cue('roll',.25,.12,arm+DISARMED_BEATS.hold+.7),cue('whoosh_heavy',.2,.18,neck-.22),cue(gore?'flesh_tear':'hit_heavy',.7,.12,neck),
          cue('kill',.65,.25,DISARMED_BEATS.settle*DISARMED_BEATS.duration+2*DISARMED_BEATS.hold));
        continue;
      }
  ...
  if (deaths.length) impacts.push(deaths.length > 1 || presentation?.finish.draw ? cue('crowd_gasp', .25, .18, .35) : finisher === 'disarmed' ? cue('crowd_cheer',.3,.16,2.2) : finisher === 'quietOne' ? cue('crowd_gasp', .16, .12, 2.8) : cue('crowd_cheer', finisher && gore ? .3 : .23, .16, .35));
