...
const VIBRATO = { rate: 4.2, depth: .015, from: .3, until: 2, fade: .6 }, BLOOM = { above: 500, seconds: .12 };   // the wobble lives in the first 2 s and settles out of the tail
...
export function bellSamples(rate: number): Float32Array {
  ...
  for (let i = 0; i < data.length; i++) { const t = i / rate, grow = wobbleAt(t, { ...VIBRATO, from: TREMOLO.from }); data[i] *= 1 - TREMOLO.depth * grow * (1 - Math.cos(2 * Math.PI * TREMOLO.rate * t)) / 2; }
