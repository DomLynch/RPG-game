...
export function bellSamples(rate: number): Float32Array {
  const data = new Float32Array(Math.round(2.6 * rate));
  for (const [hz, gain, decay] of [[110, .8, 1.7], [331, .8, 1.5], [552, .45, 1.1], [763, .12, .7], [1136, .08, .4]]) {
    for (let i = 0; i < data.length; i++) { const t = i / rate; data[i] += gain * Math.sin(t * 2 * Math.PI * hz) * Math.exp(-5 * t / decay); }
  }
  let peak = 0, square = 0;
  for (let i = 0; i < data.length; i++) {
    data[i] *= Math.min(1, i / Math.round(.003 * rate), (data.length - i - 1) / Math.round(.3 * rate));
