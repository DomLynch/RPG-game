...
export function selectFinisher(finish: Finish, weapons: readonly [WeaponId, WeaponId]): FinisherId | null {
  ...
  // Decapitation, Run Through, The Quiet One, or no ceremony at all — picked by a deterministic rotation pool seeded from the kill event
  ...
  return pick === 0 ? 'splitCrown' : pick === 1 ? 'decapitation' : pick === 2 ? 'runThrough' : pick === 3 ? 'plainDeath' : 'quietOne';
}
...
export const FINISHER_POSE: Record<FinisherId, 'splitCrown' | 'decapitation' | 'runThrough' | 'quietOne' | null> = {
