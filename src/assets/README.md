...
## Combat audio (audio lane, 2026-09-15; reconciled 2026-09-20; corrected 2026-09-22 — Auditer finding #2)
`src/assets/audio/sprite.m4a` (AAC-LC 96 kb/s, Apple AudioToolbox encoder via ffmpeg `aac_at`, for Safari) and `sprite.ogg`
(Opus 80 kb/s VBR via ffmpeg `libopus`, for Chrome/Android) are one audio sprite built by `node scripts/build-audio.mjs`
(needs ffmpeg on PATH; bit-exact flags, so two builds are byte-identical). Both rates were stepped down under the 1.0 MB
gzip lane budget as the sprite grew (three cuts since #245: AAC 128→96k, Opus 92→80k) — a swap of bitrate for headroom,
not new content; SNR is printed by `audio-preview.mjs` but not yet asserted (Auditer finding #6, open). The generated
`src/audio/manifest.ts` maps cue → variants → [start, duration]. Three kinds of source, all listed in this file:
- Original procedural Foley (deterministic Node DSP: seeded noise, broadband sweeping-low-pass impact bodies, dense gritty
  inharmonic iron clusters, swept air), normalised to −4 dBFS for codec headroom — most of whoosh_light/heavy, draw,
  hit_flesh/heavy, block, block_perfect, guard_break, charge, kill, roll, backstep; 2–7 seeded variants each. No AI
