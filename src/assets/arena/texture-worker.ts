...
import { floorOverlay, sandAlbedo, sandNormal, skyPixels, stoneAlbedo, stoneNormal, tinted, type Pixels } from './textures.ts';
...
export function generateHeavyTextures(phone: boolean, skyU: number, look: TextureLook): HeavyTextures {
  ...
  return { sand: tinted(floor === 'sand' || flag ? ground : floorOverlay(ground, floor, sandSeed), look.sand), sandNormal: flag ? stoneNormal(half(512), sandSeed) : sandNormal(half(512), sandSeed), stone: tinted(stoneAlbedo(half(512), look.stoneSeed, look.wall), look.stone), stoneNormal: stoneNormal(half(512), look.stoneSeed, look.wall), sky: skyPixels(half(512), half(256), skyU, 0.77, 19, look.sky) };
}
