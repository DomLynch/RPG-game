...
export const TINT = { strength: 0.85, minGain: 0.35, maxGain: 2.5, mapMetal: 0.6, mapRough: 0.7 };
...
export function tinted(source: MeshStandardMaterial, tier: Tier): MeshStandardMaterial {
  const finish = gradeFor(tier, source.name); if (!finish) return source;
  ...
  const chroma = grade.clone().multiplyScalar(1 / gradeLuma), gain = Math.min(TINT.maxGain, Math.max(TINT.minGain, gradeLuma / sourceLuma));
  material.onBeforeCompile = (shader) => {
    shader.uniforms.rankTint = { value: [chroma.r, chroma.g, chroma.b, gain] };
    shader.uniforms.rankStrength = { value: TINT.strength };
      ...
      .replace('#include <color_fragment>', `#include <color_fragment>
{
  ...
  const toward = (from: number, to: number) => from + (to - from) * TINT.strength;
  if (source.metalnessMap) material.metalness = Math.min(1, source.metalness * Math.min(1.7, Math.max(0.2, finish.metalness / TINT.mapMetal)));
  else material.metalness = toward(source.metalness, finish.metalness);
  if (source.roughnessMap) material.roughness = Math.min(1, source.roughness * Math.min(1.5, Math.max(0.3, finish.roughness / TINT.mapRough)));
