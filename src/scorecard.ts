  ...
  try {
    ...
    if (value?.version === 1 && value.rows && typeof value.rows === 'object')
      for (const [id, line] of Object.entries(value.rows as Record<string, Partial<Line>>))
        if (isOpponentId(id) && line) card.rows[id] = { fights: count(line.fights), wins: count(line.wins), losses: count(line.losses), left: count(line.left) };
...
export function recordResult(card: Scorecard, opponent: OpponentId, outcome: Outcome, left = false): Line {
...
export function scorecardRows(card: Scorecard, opponents: readonly { id: OpponentId; name: string }[]): { name: string; fights: number; wins: number; losses: string }[] {
  const losses = (line: Line) => (line.left ? `${line.losses} (${line.left} left)` : String(line.losses));
  const rows = opponents.map(({ id, name }) => { const line = card.rows[id] ?? emptyLine(); return { name, fights: line.fights, wins: line.wins, losses: losses(line) }; });
  const all = totals(card);
  return [...rows, { name: 'All fights', fights: all.fights, wins: all.wins, losses: losses(all) }];
}
