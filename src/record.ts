...
export const RECORD_VERSION = 6;   // 6: the estoc's reach fix (#419) — ESTOC_MOVES takes the blade's real reach (the sword's convention + 0.30 m), so every fight involving the estoc, or the Nightborn who wields it, replays differently. 5: the batched weapon-data flip (2026-09-22) — the knife's thrust recovery 15 -> 20 and the scythe's heel-jab 18 -> 30, one bump for the pair rather than one each (the lead's ruling: kill links are the viral surface, and N bumps means N waves of dead links). 4 was taken by Brief 13's whip tell while this branch was in flight, so this is 5, not the 4 the branch first wrote.
