...
export type Arena = { group: THREE.Group; floor: THREE.Mesh; readonly sky: THREE.Texture; ready: Promise<void>; update(dt: number, events: CombatEvent[], camera?: THREE.Camera, sim?: SimView): void; guards(url: string): Promise<void>; dispose(): void };
...
export function buildArena(scene: THREE.Scene): Arena {
  ...
  const lorarii = buildLorarii(group);   // the placeholder capsules; `guards(url)` below swaps in guard.glb's six bodies
  update(0, []);
  const props = loadArenaProps(group, phone, (what) => { if (what === 'gateBars') gateBars.visible = false; });
  return {
    group, floor, update, guards: lorarii.bodies, ready: Promise.all([props.ready, texturesReady]).then(() => undefined),
