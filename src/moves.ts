...
const ARCHETYPES: Record<(typeof ROSTER)[OpponentId]['archetype'], Omit<Opponent, 'id' | 'weapon' | 'rig'>> = {
  ...
  veteran: { scale: 1, health: RULES.health, poise: 0, profiles: { ...PROFILES, hard: { ...PROFILES.hard, pressure: .7, discipline: 30 } }, opener: true },   // the trident since slice V (2026-09-16); opener: brief 8's authored opening (2026-09-22) — one side, the other side, a punishable recovery, then this same table takes over. The archetype, not an id check (roster.ts's own rule: an individual must not add an AI branch) — the skeleton (roster.ts, season-2, held) shares this archetype and so shares the opener too; nothing else does.
