...
let finisherOverride: FinisherId | null = null;   // dev/test pick (owner 2026-09-19): swap which finisher plays on a ceremonial kill; null = the spec's selection
  ...
  return {
    ...
    bloodState() { const opened = warriors?.opponent.anchor.getObjectByName('Opened'); return {...finisherBlood.inspect(),head:severHead ? {position:severHead.group.position.toArray(),screen:this.project(severHead.group.position.toArray()),visible:severHead.group.visible} : null,opened:opened ? {visible:opened.visible,pieces:opened.children.filter(p=>p.name!=='OpenedWeapon').map(p=>({name:p.name,visible:p.visible,opacity:((p.getObjectByName('CreatureBody') as THREE.Mesh | undefined)?.material as THREE.MeshStandardMaterial | undefined)?.opacity ?? 1}))} : null,sources:bloodSources.map(s=>({site:s.site,position:s.position.toArray(),direction:s.direction.toArray()}))}; },
