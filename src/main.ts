...
function frame(now: number) {
  ...
  if (!paused()) {
    ...
    while (accumulator >= step()) {
      ...
      feedback.update(practice.events, deathAudio, { match: matchSeed, ended: !!practice.finish, tick: practice.duel.tick, drawing: practice.duel.fighters[0].phase === 'draw', opponent: opponent.id }); frameEvents.push(...practice.events); floatDamage(practice.events);
