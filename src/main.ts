...
const FINISHER_OPTIONS: [string, string][] = [['splitCrown', 'Split Crown'], ['decapitation', 'Decapitation'], ['runThrough', 'Run Through'], ['quietOne', 'The Quiet One'], ['opened', 'Opened'], ['plainDeath', 'Plain death']];
...
element('name-form').addEventListener('submit', event => {
  event.preventDefault(); profile.name = cleanName(input.value); persist(); welcome.hidden = true; clearInput(); feedback.unlock(); canvas.focus();
});
...
function frame(now: number) {
  ...
  if (!paused()) {
    ...
    while (accumulator >= step()) {
      ...
      const deathAudio = practice.finish && practice.events.some(e => e.type === 'Killed') ? { finish: practice.finish, weapons: [practice.duel.fighters[0].weapon, practice.duel.fighters[1].weapon] as const, override: resolveFinisher(opponent.id, practice.finish, [practice.duel.fighters[0].weapon, practice.duel.fighters[1].weapon], finisherSelect.value === 'auto' ? null : finisherSelect.value as FinisherId) ?? 'plainDeath', gore: bloodMode !== 2 } : undefined;
      feedback.update(practice.events, deathAudio, { match: matchSeed, ended: !!practice.finish, tick: practice.duel.tick, opening: practice.duel.fighters[0].phase === 'sheathed' || practice.duel.fighters[0].phase === 'draw' }); frameEvents.push(...practice.events); floatDamage(practice.events);
  ...
  updateHud();
  if (debug) { const d = element('debug'); d.textContent = describe(practice, difficulty); d.dataset.frozen = String(hitStop > 0); d.dataset.tick = String(practice.duel.tick); d.dataset.tip = (view.bladeTip?.() ?? []).map(v => v.toFixed(4)).join(','); d.dataset.clips = view.playing?.() ?? ''; d.dataset.blood = JSON.stringify(view.bloodState()); }   // frame probe: frozen flag, tick, drawn blade tip, the clip each rig plays
