...
function began() {
  ...
  showAutopsy([]); lootPanel.hide(); match.frameEvents = []; shareButton.hidden = true; say(null); updateHud();
}
    ...
    while (accumulator >= step()) {
      ...
      if (result === 'ended') {
        ...
        if (match.replay) { banner(`Replay over · ${practice.finish?.victim === 1 ? `${ROSTER[opponent.id].name} fell` : 'the fighter fell'}`); updateHud(); }   // a watched fight is never a walk-away
          ...
          if (ended.rewarded && ended.won) { offerLoot(Math.max(0, Math.round(practice.playerHealth))); persist(); }
