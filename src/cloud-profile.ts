...
export type CloudProfile = { display_name: string; encounter: OpponentId | null; revision: number; victory_marks: number };
const columns = 'display_name,encounter,revision,victory_marks';
export function cloudProfile(value: unknown): CloudProfile {
  ...
  if (!row || typeof row.display_name !== 'string' || row.display_name !== cleanName(row.display_name)
    ...
    || !Number.isSafeInteger(row.victory_marks) || row.victory_marks! < 0) throw Error('Invalid saved fighter');
...
export function fighterDetails(profile: Profile) {
  return { display_name: cleanName(profile.name), encounter: profile.encounter ?? null, victory_marks: profile.career?.victoryMarks ?? 0 };
}
