...
import { prepareDisarmed, DISARMED_BEATS } from './disarmed.ts';
...
const ONE_SHOT: readonly Role[] = ['Attack', 'Hit', 'Death', 'Draw', 'Roll', 'Guard', 'Return', 'Heavy', 'Riposte', 'Thrust', 'Kick', 'BlockImpact', 'Parry', 'Deflected', 'Death_SplitCrown', 'Death_RunThrough', 'Fin_RunThrough', 'Death_QuietOne', 'Death_Disarmed', 'Fin_Disarmed'];
...
function fighterClips(asset: FighterAsset, weapon: WeaponId): Record<Role, AnimationClip> {
  ...
  for (const role of [...ROLES, ...ADDITIVE_ROLES]) {
    ...
    // Additive scenes may be absent on rigs that have not passed their anatomy review.
    if (!clip && (role === 'Death_Disarmed' || role === 'Fin_Disarmed')) continue;
...
export function buildWarriors(asset: FighterAsset, opponentAsset?: FighterAsset, weapons: [WeaponId, WeaponId] = ['longsword', 'longsword']) {
  ...
  function create(opponent: boolean) {
    ...
    const roles = [...ROLES, ...ADDITIVE_ROLES].filter(role => clips[role]);
    ...
    let disarmed: ReturnType<typeof prepareDisarmed> | undefined;
    ...
    return {
      ...
      update(travelSpeed: number, dt: number, pose: 'sheathed' | 'draw' | 'ready' | 'attack' | 'hit' | 'death' | 'splitCrown' | 'decapitation' | 'runThrough' | 'runThroughHold' | 'quietOne' | 'opened' | 'disarmed' | 'disarmedStrike' | 'roll' | 'guard' | 'kick' | 'block' | 'parry' | 'deflected' = 'sheathed', progress = 0, attack: Attack = 'light', contact = .35, lateral = 0, recoil = 0) {
        ...
        if ((pose === 'disarmed' || pose === 'disarmedStrike') && !clips[pose === 'disarmed' ? 'Death_Disarmed' : 'Fin_Disarmed']) throw new Error('Disarmed clips are not installed on this rig');
        if (pose !== 'disarmed') disarmed?.apply(0, 'off');
        ...
        const combatRole: Role | null = pose === 'block' ? 'BlockImpact' : pose === 'parry' ? 'Parry' : pose === 'deflected' ? 'Deflected' : pose === 'kick' ? 'Kick' : pose === 'attack' ? attack === 'return' ? 'Return' : attack === 'heavy' ? 'Heavy' : attack === 'riposte' ? 'Riposte' : attack === 'thrust' ? 'Thrust' : 'Attack' : pose === 'hit' ? 'Hit' : pose === 'death' ? 'Death' : pose === 'splitCrown' || pose === 'decapitation' || pose === 'opened' ? 'Death_SplitCrown' : pose === 'runThrough' ? 'Death_RunThrough' : pose === 'disarmed' ? 'Death_Disarmed' : pose === 'disarmedStrike' ? 'Fin_Disarmed' : pose === 'quietOne' ? 'Death_QuietOne' : pose === 'runThroughHold' ? 'Fin_RunThrough' : pose === 'draw' ? 'Draw' : pose === 'roll' ? 'Roll' : pose === 'guard' ? 'Guard' : null;
        const armed = pose !== 'sheathed';
        if (armed) { weights.Armed = weights.Idle; weights.Idle = 0; }
        const dead = pose === 'death' || pose === 'splitCrown' || pose === 'decapitation' || pose === 'runThrough' || pose === 'quietOne' || pose === 'opened' || pose === 'disarmed';
        for (const role of roles) {
          ...
          a.setEffectiveWeight((pose === 'opened' || (['disarmed','disarmedStrike'].includes(pose) && progress >= .05)) ? target : activeBlade ? Number(role === combatRole) : a.getEffectiveWeight() + (target - a.getEffectiveWeight()) * (1 - Math.exp(-step * 24)));
      ...
      prepareDisarmed() {
        if (disarmed) return;
        if (!clips.Death_Disarmed) throw new Error('Disarmed victim clip is not installed');
        const saved = roles.map(role => ({role, time:actions[role].time, weight:actions[role].getEffectiveWeight()}));
        const shown = [blade.visible, sheathed?.visible], position = root.position.clone(), rotation = root.quaternion.clone();
        for (const role of roles) actions[role].setEffectiveWeight(Number(role === 'Death_Disarmed'));
        actions.Death_Disarmed.time = clips.Death_Disarmed.duration * DISARMED_BEATS.arm;
        mixer.update(0); root.position.set(0,0,0); root.quaternion.identity();
        blade.visible = true; if (sheathed) sheathed.visible = false;
        try { disarmed = prepareDisarmed(root, anchor); anchor.add(disarmed.group); }
        finally {
          for (const state of saved) { actions[state.role].time = state.time; actions[state.role].setEffectiveWeight(state.weight); }
          mixer.update(0); root.position.copy(position); root.quaternion.copy(rotation);
          blade.visible = shown[0]!; if (sheathed) sheathed.visible = shown[1]!;
