...
export function createFootDust(scene: THREE.Scene) {
  const count = 24, positions = new Float32Array(count * 3), fades = new Float32Array(count), sizes = new Float32Array(count);
  ...
  for (let y = 0; y < 32; y++) for (let x = 0; x < 32; x++) {
    ...
    pixels.set([255, 255, 255, 255 * edge ** 2 * (0.65 + 0.35 * Math.sin(x * 0.9 + Math.sin(y * 0.6)) ** 2)], i);
  }
  ...
  const material = new THREE.PointsMaterial({ map, color: '#978568', size: 0.22, opacity: 0.2, transparent: true, depthWrite: false });
  ...
  return {
    update(dt: number, feet: readonly (THREE.Vector3 | null)[], moving: readonly boolean[]) {
      ...
      feet.forEach((foot, i) => {
        ...
        if (foot && old && moving[i] && cooldown[i] === 0 && foot.y < 0.18 && foot.y < old.y - 0.0005 && foot.distanceTo(old) < 0.3) {
          ...
          for (let j = 0; j < 3; j++) {
            ...
            life[p] = 0.55; sizes[p] = 0.65;
            positions[p * 3] = foot.x + Math.sin(angle) * 0.045; positions[p * 3 + 1] = 0.035; positions[p * 3 + 2] = foot.z + Math.cos(angle) * 0.045;
            velocity[p * 3] = Math.sin(angle) * 0.12; velocity[p * 3 + 1] = 0.065; velocity[p * 3 + 2] = Math.cos(angle) * 0.12;
          }
        }
      ...
      for (let p = 0; p < count; p++) {
        life[p] = Math.max(0, life[p] - dt); const age = 0.55 - life[p];
        fades[p] = Math.min(1, age / 0.06) * life[p] / 0.55; sizes[p] = 0.65 + age * 1.3;
        if (life[p] > 0) { active = true; for (let axis = 0; axis < 3; axis++) positions[p * 3 + axis] += velocity[p * 3 + axis] * dt; }
      }
