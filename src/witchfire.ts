...
export const WITCHFIRE = { windup: MOVES.skill_witchfire.windup, active: MOVES.skill_witchfire.active, reach: MOVES.skill_witchfire.reach, cone: 0.42 } as const;   // cone: half-angle, radians
...
export function createWitchfire(scene: THREE.Scene) {
  const positions = new Float32Array(POOL * 3), colors = new Float32Array(POOL * 3), sizes = new Float32Array(POOL);
  ...
  material.customProgramCacheKey = () => 'witchfire-v1';
  ...
  const emit = (k: number, speed: number, spread: number, lifeMin: number, lifeMax: number, size: number, rise: number) => {
    ...
    const s = speed * (0.75 + random() * 0.5);
    for (let axis = 0; axis < 3; axis++) { positions[p * 3 + axis] = palm.getComponent(axis); velocity[p * 3 + axis] = dir.getComponent(axis) * s; }
    ...
    update(dt: number, fighters: readonly Fighter[], roots: readonly (THREE.Object3D | null)[]) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (stage === 'windup') {
          ...
          for (let n = 0; n < ticks * 16; n++) emit(0, movesOf(f).skill_witchfire.reach * 8, WITCHFIRE.cone, 0.26, 0.42, 0.34, 0.05);
        } else {
      ...
      if (dt > 0) {
        ...
        for (let p = 0; p < POOL; p++) {
          ...
          for (let axis = 0; axis < 3; axis++) positions[v + axis] += velocity[v + axis] * dt;
          if (positions[v + 1] < 0.02) { positions[v + 1] = 0.02; velocity[v + 1] = 0; }
          ...
          colors[v] = (0.03 + 0.12 * hot) * gain; colors[v + 1] = (0.12 + 0.3 * heat) * gain; colors[v + 2] = (0.04 + 0.08 * hot) * gain;
          const scale = kind[p] === 0 ? grow[p] * (0.45 + 1.5 * (1 - heat)) : grow[p] * (0.5 + 0.5 * heat);   // the gout billows as it goes
          sizes[p] = scale * Math.min(1, heat * 3);
        }
      } else for (let p = 0; p < POOL && !active; p++) active = life[p] > 0;   // a pause holds the flames where they are
      if (active && dt > 0) for (const attribute of Object.values(geometry.attributes)) attribute.needsUpdate = true;
