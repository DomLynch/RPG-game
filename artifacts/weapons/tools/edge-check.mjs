...
for (const [kind,spec] of Object.entries(PATHS)) { if (kind.endsWith('_chain')) continue;
 ...
 const at=age=>{ mixer.setTime(Math.min(.999999,swingProgress(age/n,spec.windup/n,spec.source))*clip.duration); asset.scene.updateMatrixWorld(true); return { tip: blade.localToWorld(new Vector3(0,(blade.userData?.contact?.to ?? .86),0)), q: blade.getWorldQuaternion(new Quaternion()) }; };
