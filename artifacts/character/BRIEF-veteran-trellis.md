...
## Plan
1. Owner generates the sheet and the reconstruction source from `docs/character-references/PROMPTS.md` §Veteran with his
   seven portraits attached; saves as `docs/character-references/veteran-v2.png` and
   `src/assets/source/creatures/veteran-front.png`.
2. `python scripts/character/trellis.py src/assets/source/creatures/veteran-front.png src/assets/source/creatures/veteran.glb`
   (seed 190926, 1024, 100k faces, 2048 texture — the creatures' settings). Needs `HF_TOKEN` in `.env.hf.local`
   (git-ignored): anonymous ZeroGPU quota is zero (tested 2026-09-20); the owner's HF account is PRO.
