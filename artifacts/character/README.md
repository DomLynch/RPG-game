...
`humanoid-v1…v5` (iterations; v5 = weapon pass, committed 131cd52) · `motion-v1` (UAL2 Guard sheet) · `ual2-attacks` (opt-in strike candidates + variant GLB) · `ual2-block` (raw UAL2 key frames) · `level1-kit-raw` · `moodboard` (direction pick) · `realistic-v1` (Blender Studio head, painted) · `realistic-v2` (face tile, sculpted normal, hair/brow/lash cards; `face.png`, `eyes-zoom.png`) · `proof-*` (pipeline proofs, deleted).
