...
- First landing (#179, 79cf423) was reverted (63f4cd9): release check 20/34 `creature-check` — the Minotaur and Werewolf are creature bakes on the Pitborn base and record its sha256, so the new `pitborn.glb` made them "stale". Re-landed with both rebuilt from the new base (`build-creatures.mjs minotaur` / `werewolf`, bind error 2.2e-6, 25 clips each; `bake-blades` left `blade-paths.ts` byte-identical — the maul path is unchanged). **Proposed rule (from the lead's revert note, pending the owner's go/no-go):** a PR that changes a base rig ships with the rebuilt creatures that derive from it — this is what `creature-check`'s base sha256 already enforces mechanically; whether the character lane or the creature lane owns those rebuilds is the owner's call. Receipts: creature-check 0, creature-browser-check 0, budget PASS (29.89 MB), 309/309 + browser gate.
