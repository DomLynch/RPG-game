...
## Veteran neck and trident follow-up — 2026-09-19 (CPU candidate, not published)
Owner requests a seamless neck and more realistic weapon, preserving the existing Veteran. The offline finish now fits a continuous collar contour, removes the 1.5 mm overlap, and shares collar skin weights across the two surfaces. Skin colour/normal/roughness transitions are baked into existing UVs from the retained original maps. The trident reuses the reviewed worn-bronze maps with restrained normal relief; its geometry, UVs, reach and clips remain exact. Facial features and all surfaces outside the collar remain geometrically unchanged.

The initial small fit left a pinched nape; a wider weight experiment opened the shoulder join and was rejected. Final contour smoothing fixes the nape without changing that lower boundary. CPU regression samples all 38 clips (191 poses), measuring a maximum 0.0011 mm head/neck separation. Full rebuild and repeated material bake are byte-identical; preservation/source-map audits pass. CPU quality passes 294 tests, lint/typecheck/build/audit and asset budgets. Final audit corrected stale accessor bounds; the added regression fails before and passes after, with all binary geometry/texture payloads unchanged. Current GLB SHA256: `6101410cd0d2ca4800ef86b8fb96fec49bb174aaff0d01e09f2a85519b01b3cb`. Evidence: `artifacts/character/veteran-neck/`. Matched CPU renders are review previews, not game/browser proof. PR172 remains draft; lead explicitly holds browser gates, trunk merge and publication pending a coordinated GPU/release slot. Full completion/browser gates and live parity remain open; no all-clear or deployment claim.
