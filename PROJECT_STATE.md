...
## Hamstrung authoring draft — 2026-09-19
Owner-authorized second scene: low cut, knee buckle, folded upper-back thrust with an embedded ending. Offline paired motion passes actual eight-rig grounding, joint-continuity, source-restoration and held-pose checks. The first low-cut check failed because crouching used rig-local units; world-space lowering with planted feet passes the same check. A shared append-only writer preserves earlier clips and rejects later foreign animations; both Disarmed and Hamstrung serialization checks pass. No Hamstrung GLB, menu or runtime enable yet. Disarmed visual sign-off remains queued; visual audits and publication stay in owner order. Existing creature Opened/front-Decap repair is publicly verified at3803433; new scenes are not live.

## Disarmed draft — 2026-09-19 (not published)
First of the owner's eight-scene queue: Disarmed, Hamstrung, Cleave, Underfoot, Reversal, Crosscut, Drive, Rising Death. Disarmed adds paired clips to the eight current rigs without changing prior animation/binary data. The actual forearm and held weapon separate at the first impact; the victim clutches the stump before the neck strike and grounded fall. Actor sampling calculates both blade contacts without advancing the live pose or ghost clock. Cut geometry is prepared outside combat; bounded blood, two impact holds, delayed sound beats, mode/reset cleanup and spectral pieces are wired in the isolated draft. The journal can select it; Auto stays unchanged until visual acceptance. Per-body victim tracks are independently checked rather than incorrectly required to match the hero's fall. Creature motion/arm/contact and ghost-resource checks pass locally. Rendered foot placement, full camera/contact sequence, audio listening, full gates and public verification are pending. Existing repair PR167 and the ordered audio/character releases publish first; do not call this scene live.
